...
public class FileGeneratorTest {
  ...
  static {
    features.put(
        "element", asList(new Value(null, "foo"), new Value(null, "bar"), new Value(null, "baz")));
    features.put(
        "attribute",
        asList(new Value(null, "foo"), new Value(null, "bar"), new Value(null, "baz")));
  }
  ...
  public void testGenerate() throws IOException, SAXException {
    ...
    assertXMLEqual(
        new InputSource(
            new File(resourceDir, "exp" + File.separator + "dummy.xml").toURI().toString()),
  ...
  public static class ElementAction extends AbstractAction {
    ...
    public void getResult(ContentHandler output) throws SAXException {
      ...
      output.startElement(
          NULL_NS_URI, "foo", "foo", new AttributesBuilder().add("bar", "baz").build());
