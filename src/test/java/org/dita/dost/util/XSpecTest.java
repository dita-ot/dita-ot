...
public class XSpecTest {
  ...
  public static void setUpClass() throws TransformerException {
    ...
    final File ditaDir =
        new File(
                Optional.ofNullable(System.getProperty("dita.dir"))
  ...
  public void testXSpec() throws TransformerException, ParserConfigurationException {
    ...
    final Transformer compiledXspec =
        transformerFactory.newTransformer(new DOMSource(stylesheet.getNode()));
    ...
    for (int i = 0; i < tests.getLength(); i++) {
      ...
      final boolean res =
          Boolean.parseBoolean(test.getAttribute("successful"))
              || test.getAttributeNode("pending") != null;
      if (!res) {
        final Element scenario = (Element) test.getParentNode();
        final Node label =
            scenario.getElementsByTagNameNS(XSPEC_NS, "label").item(0).getFirstChild();
        final String act =
            ((Element) scenario.getElementsByTagNameNS(XSPEC_NS, "result").item(0))
                .getAttribute("select");
        final String exp =
            ((Element) test.getElementsByTagNameNS(XSPEC_NS, "expect").item(0))
                .getAttribute("select");
