...
public class FilterUtilsTest {
  ...
  public void testgetFlagsDefaultFlag() {
    ...
    assertEquals(
        singleton(flag), f.getFlags(attr(PLATFORM, "amiga unix windows"), new QName[0][0]));
  }
  ...
  public void testExtCheckExclude() {
    ...
    assertFalse(
        f.extCheckExclude(new QName[] {PLATFORM, OS}, Arrays.asList("amiga", "unix", "windows")));
    assertTrue(f.extCheckExclude(new QName[] {PLATFORM, OS}, Arrays.asList("osx")));
    assertFalse(
        f.extCheckExclude(new QName[] {PLATFORM, OS}, Arrays.asList("unix", "amiga", "windows")));
  }
  ...
  public void testNeedExcludeGroupMultiple() {
    ...
    assertFalse(
        f.needExclude(attr(PLATFORM, "os(amiga unix windows) database(mongo)"), new QName[0][0]));
    assertTrue(f.needExclude(attr(PLATFORM, "os(amiga windows) database(mongo)"), new QName[0][0]));
    assertFalse(
        f.needExclude(attr(PLATFORM, "gui(amiga windows) database(mongo)"), new QName[0][0]));
    assertTrue(f.needExclude(attr(PLATFORM, "os(windows) database(mongo)"), new QName[0][0]));
    assertTrue(
        f.needExclude(
            attr(PLATFORM, "   os(   windows   )   database(  mongo  )   "), new QName[0][0]));
  }
  ...
  public void testNeedExcludeMixedGroups() {
    ...
    assertFalse(
        f.needExclude(
            attr(PLATFORM, "windows database(mongodb couchbase) unix osx"), new QName[0][0]));
    assertTrue(
        f.needExclude(attr(PLATFORM, "windows database(mongodb couchbase) unix"), new QName[0][0]));
