...
//        resourceDir = TestUtils.getResourceDir(getClass());
//        expBaseDir = new File(resourceDir, "exp" + File.separator + testCase);
    }
...
//        logger = new TestUtils.CachingLogger(true);
...
//        chunkModule.setParallel(parallel);
        ...
        for (File file : tempDir.listFiles((dir, name) -> name.endsWith("dita") || name.endsWith("ditamap"))) {
            ...
            for (Node n : getChildren(d)) {
                if (n.getNodeType() == Node.PROCESSING_INSTRUCTION_NODE && n.getNodeName().equals("workdir-uri")) {
                    n.setNodeValue(tempDir.toURI().toString());
                    break;
                }
            }
...
//    public void setUp() throws Exception {
//        tempBaseDir = TestUtils.createTempDir(getClass());
//        TestUtils.copy(new File(resourceDir, "src" + File.separator + testCase), tempBaseDir);
//
//        tempDir = new File(tempBaseDir, testCase);
//        chunkModule = getModule(tempDir);
...
//
//        final File expDir = Paths.get(testDir.getAbsolutePath(), EXP_DIR, "preprocess", testCase).toFile();
//        final File actDir = Paths.get(baseTempDir.getAbsolutePath(), testDir.getName(), "temp", "preprocess", testCase).toFile();
...
//            project.init();
//            project.setUserProperty("test-name", testCase);
