...
public class TestGenMapAndTopicListModule {
  ...
  private static final File resourceDir =
      TestUtils.getResourceDir(TestGenMapAndTopicListModule.class);
  ...
  private static Job generate(
      final File inputDir, final File inputMap, final File outDir, final File tempDir)
    ...
    pipelineInput.setAttribute(
        ANT_INVOKER_EXT_PARAM_GENERATECOPYOUTTER, Integer.toString(NOT_GENERATEOUTTER.type));
    ...
    pipelineInput.setAttribute(
        ANT_INVOKER_PARAM_MAPLINKS, new File(tempDir, "maplinks.unordered").getPath());
  ...
  public void testFileContentParallel() throws Exception {
    ...
    assertEquals(
        new HashSet<>(List.of("maps/root-map-01.ditamap")),
    ...
    assertEquals(
        new HashSet<>(List.of("topics/xreffin-topic-1.xml")),
  ...
  public void testFileContentAbove() throws Exception {
    ...
    assertEquals(Collections.emptySet(), readLines(new File(e, "flagimage.list")));
    assertEquals(
        new HashSet<>(List.of("root-map-02.ditamap")), readLines(new File(e, "fullditamap.list")));
    assertEquals(
        new HashSet<>(
            Arrays.asList(
                "topics/xreffin-topic-1.xml",
    ...
    assertEquals(Collections.emptySet(), readLines(new File(e, "subtargets.list")));
    assertEquals(
        new HashSet<>(List.of("root-map-02.ditamap")),
        readLines(new File(e, "usr.input.file.list")));
