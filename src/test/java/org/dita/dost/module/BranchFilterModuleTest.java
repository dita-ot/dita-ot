...
public class BranchFilterModuleTest extends BranchFilterModule {
  ...
  private Job getJob() throws IOException {
    ...
    for (final String uri :
        Arrays.asList(
            "linux.ditaval", "novice.ditaval", "advanced.ditaval", "mac.ditaval", "win.ditaval")) {
      job.add(
          new Job.FileInfo.Builder()
  ...
  //    @Test
  //    public void testSplitBranches() throws ParserConfigurationException, IOException,
  // SAXException {
  ...
  public void testProcessMap() throws IOException, SAXException {
    ...
    final List<String> filesAct =
        Arrays.stream(tempDir.listFiles((dir, name) -> name.endsWith(".dita")))
            .map(File::getName)
            .sorted()
  ...
  public void testUplevelsMap() throws IOException, SAXException {
    ...
    assertXMLEqual(
        new InputSource(new File(uplevelsExpDir, "main/testuplevels.ditamap").toURI().toString()),
    ...
    final List<String> filesExpMain =
        Arrays.asList(
            "main/kiddo.dita",
            "main/kiddodefault.dita",
            "main/parent.dita",
            ...
            "main/weechilddefault-1.dita",
            "main/weechild-novice.dita");
    final List<String> filesExpMainSubdir =
        Arrays.asList(
            "main/subdir/PREFIX-weechild-advanced.dita",
            "main/subdir/weechilddefault-1.dita",
            "main/subdir/weechild-novice.dita");
    final List<String> filesExpPeer =
        Arrays.asList(
            "peer/peerkiddo.dita",
            "peer/peerkiddodefault.dita",
            "peer/peerkiddodefault-1.dita",
    ...
    final List<String> filesActMain =
        Arrays.stream(
                new File(uplevelsTempDir + File.separator + "main")
                    .listFiles((dir, name) -> name.endsWith(".dita")))
            .map(f -> "main/" + f.getName())
            .collect(Collectors.toList());
    final List<String> filesActMainSubdir =
        Arrays.stream(
                new File(uplevelsTempDir + File.separator + "main/subdir")
                    .listFiles((dir, name) -> name.endsWith(".dita")))
            .map(f -> "main/subdir/" + f.getName())
            .collect(Collectors.toList());
    final List<String> filesActPeer =
        Arrays.stream(
                new File(uplevelsTempDir + File.separator + "peer")
                    .listFiles((dir, name) -> name.endsWith(".dita")))
  ...
  public void testRewriteReferencesContainingAnchors() throws IOException, SAXException {
    ...
    assertXMLEqual(
        new InputSource(new File(expDir, "topicAnchors.ditamap").toURI().toURL().toString()),
  ...
  public void testNoRenameNonDITAResources() throws IOException, SAXException {
    ...
    assertXMLEqual(
        new InputSource(new File(expDir, "inputNonDitaRes.ditamap").toURI().toString()),
