...
public class IncludeResolverTest {
  ...
  public void setup() throws IOException {
    ...
    for (final String file :
        new String[] {"code.xml", "utf-8.xml", "plain.txt", "range.txt", "schema.xml"}) {
    ...
    job.addAll(
        Stream.of("code.xml", "utf-8.xml", "plain.txt", "range.txt", "schema.xml")
            .map(
                p ->
                    new Builder()
                        .uri(create(p))
  ...
  //        filter.write(f.getAbsoluteFile());
  //
  //        assertXMLEqual(new InputSource(new File(expDir,
  // "include_text.dita").toURI().toString()),
  //                new InputSource(f.toURI().toString()));
  //    }
  //
  //    @Test
  ...
  //
  //        assertXMLEqual(new InputSource(new File(expDir,
  // "include_dita.dita").toURI().toString()),
  //                new InputSource(f.toURI().toString()));
  //    }
