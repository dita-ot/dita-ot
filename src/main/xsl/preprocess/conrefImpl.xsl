                      ...
                      <xsl:choose>
                        <xsl:when test="$target">
                          ...
                          <xsl:choose>
                            ...
                            <xsl:when test="document($EXPORTFILE, $current-element)//file[@name=$FILENAME]/topicid[@name=$firstTopicId] and ($TRANSTYPE = 'eclipsehelp')">
                              <!-- just copy -->
                              <xsl:copy>
                                <xsl:apply-templates select="*|@*|comment()|processing-instruction()|text()">
                                  <xsl:with-param name="current-relative-path" select="$current-relative-path"/>
...
-->
  ...
  <xsl:template name="checkValid">
    ...
    <xsl:variable name="input" select="tokenize($sourceDomains, '\)\s*?')"/>
    ...
    <!-- break string into node-set -->
    <!--xsl:variable name="subDomains" select="reverse(tokenize($sourceDomains, '\(|\)\s*?\(|\)' ))"/-->
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          ...
          <xsl:when test="count($sDomains)>1 and (contains($targetDomains, $constraintItem) or 
            ...
            <!-- move to next item -->
            <xsl:variable name="remainsItem" select="remove($sDomains, 1)"/>
