...
/**
 * @author george@oxygenxml.com A parser configuration that adds in a module to add Relax NG
...
public class RelaxDefaultsParserConfiguration extends XIncludeAwareParserConfiguration {
  ...
  /**
   * An XML component that adds default attribute values by looking into a Relax NG schema that
   * includes a:defaultValue annotations. See the Relax NG DTD compatibility specification.
   */
   ...
   */
  public RelaxDefaultsParserConfiguration(
      SymbolTable symbolTable, XMLGrammarPool grammarPool, Resolver resolver) {
  ...
  public boolean parse(boolean complete) throws XNIException, IOException {
    if (fInputSource != null) {
      try {
        setFeature(
            DYNAMIC_VALIDATION, getFeature(VALIDATION) || getFeature(XERCES_SCHEMA_VALIDATION));
      } catch (Exception ex) {
  ...
  /**
   * @see org.apache.xerces.parsers.XIncludeAwareParserConfiguration#setFeature(java.lang.String,
   *     boolean)
   */
  @Override
