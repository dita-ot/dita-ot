...
public class CacheStore extends AbstractStore implements Store {
  ...
  private Source toSource(final Entry entry, final URI path) {
    if (entry.doc != null) {
      ...
      if (underlyingNode.getSystemId().equals(path.toString())) {
        ...
        //                final TreeInfo rebasedDocument = new
        // RebasedDocument(underlyingNode.getTreeInfo(),
  ...
  private static class Entry {
    ...
    private Entry(
        final Document doc, final XdmNode node, final byte[] bytes, final long lastModified) {
      this.doc = doc;
      this.node = node;
      this.bytes = bytes;
