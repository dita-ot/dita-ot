...
public final class IndexTerm implements Comparable<IndexTerm> {
  ...
  /**
   * The boolean to show whether current term is leaf term leaf means the current indexterm element
   * doesn't contains any subterms or only has "index-see" or "index-see-also" subterms.
   */
  ...
  public void addSubTerm(final IndexTerm term) {
    ...
    if (IndexTermPrefix.SEE != term.getTermPrefix()
        && IndexTermPrefix.SEE_ALSO != term.getTermPrefix()) {
  ...
  public boolean equals(final Object o) {
    ...
    eqTermName =
        Objects.equals(termName, it.getTermName())
            || termName != null && termName.equals(it.getTermName());
    eqTermPrefix =
        Objects.equals(termPrefix, it.getTermPrefix())
            || termPrefix != null && termPrefix.equals(it.getTermPrefix());
    eqTermKey =
        Objects.equals(termKey, it.getTermKey())
            || termKey != null && termKey.equals(it.getTermKey());
    eqTargetList =
        targetList == it.getTargetList()
            || targetList != null && targetList.equals(it.getTargetList());
    eqSubTerms =
        subTerms == it.getSubTerms() || subTerms != null && subTerms.equals(it.getSubTerms());
  ...
  /**
   * check whether this term is leaf term leaf means the current indexterm element doesn't contains
   * any subterms or only has "index-see" or "index-see-also" subterms.
   *
   * @return boolean
   */
