...
final class InsertCatalogActionRelative extends InsertAction {
  ...
  public void startElement(final String uri, final String localName, final String qName, final Attributes attributes)
    ...
    for (int i = 0; i < attLen; i++) {
      ...
      if (
        (
          ("public".equals(localName) || "system".equals(localName) || "uri".equals(localName)) &&
        ...
        if (index == -1) {
          ...
          final String path =
            FileUtils.getFullPathNoEndSeparator(
              FileUtils.getRelativeUnixPath(paramTable.get(FileGenerator.PARAM_TEMPLATE), targetFile.toString())
            ) +
            "/";
          final String filename = FileUtils.getName(
            FileUtils.getRelativeUnixPath(paramTable.get(FileGenerator.PARAM_TEMPLATE), targetFile.toString())
          );
          attrBuf.addAttribute("http://www.w3.org/XML/1998/namespace", "base", "xml:base", attributes.getType(i), path);
