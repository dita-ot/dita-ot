...
/**
 * InsertCatalogActionRelative inserts the children of the root element of an XML document into a
 * plugin extension point, rewriting relative file references so that they are still correct in
 * their new location.
 *
 * <p>Attributes affected: (public|system|uri)/@uri
 * (nextCatalog|delegateURI|delegateSystem|delegatePublic)/@catalog
 * (rewriteSystem|rewriteURI)/@rewritePrefix To do: Handle xml:base.
 *
  ...
  public void startElement(
    ...
    for (int i = 0; i < attLen; i++) {
      ...
      final File targetFile =
          new File(new File(currentFile).getParentFile(), attributes.getValue(i));
