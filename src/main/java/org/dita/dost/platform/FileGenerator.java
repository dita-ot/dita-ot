...
final class FileGenerator extends XMLFilterImpl {
   ...
   */
  public FileGenerator(
      final Hashtable<String, List<Value>> featureTbl, final Map<String, Features> pluginTable) {
  ...
  private File removeTemplatePrefix(final File templateFile) {
    ...
    if (i != -1) {
      ...
      throw new IllegalArgumentException(
          "File " + templateFile + " does not contain template prefix");
    }
  }
    ...
    try {
      if (DITA_OT_NS.equals(uri) && EXTENSION_ELEM.equals(localName)) {
        final IAction action =
            (IAction) Class.forName(attributes.getValue(BEHAVIOR_ATTR)).newInstance();
        ...
        final Map<String, String> extensions =
            parseExtensions(attributes.getValue(DITA_OT_NS, EXTENSION_ATTR));
            ...
            if (!(EXTENSION_ATTR.equals(name))) {
              if (extensions.containsKey(name)) {
                ...
                final List<Value> value =
                    Stream.of(attributes.getValue(i).split(Integrator.FEAT_VALUE_SEPARATOR))
