...
/**
 * Integrator is the main class to control and excute the integration of the toolkit and different
 * plug-ins.
 *
...
public final class Integrator {
  ...
  @Deprecated private File propertiesFile;
  ...
  public void execute() throws Exception {
    ...
    if (!properties.containsKey(CONF_PLUGIN_DIRS)) {
      properties.setProperty(
          CONF_PLUGIN_DIRS, configuration.getOrDefault(CONF_PLUGIN_DIRS, "plugins;demo"));
    }
    if (!properties.containsKey(CONF_PLUGIN_IGNORES)) {
      properties.setProperty(
          CONF_PLUGIN_IGNORES, configuration.getOrDefault(CONF_PLUGIN_IGNORES, ""));
    }

    // Get the list of plugin directories from the properties.
    final String[] pluginDirs =
        properties.getProperty(CONF_PLUGIN_DIRS).split(PARAM_VALUE_SEPARATOR);
    ...
    for (final String tmpl :
        properties.getProperty(CONF_TEMPLATES, "").split(PARAM_VALUE_SEPARATOR)) {
    ...
    for (final String pluginDir2 : pluginDirs) {
      ...
      for (int i = 0; (pluginFiles != null) && (i < pluginFiles.length); i++) {
        ...
        if (pluginFiles[i].isDirectory()
            && !pluginIgnores.contains(f.getName())
            && descFile.exists()) {
  ...
  private void integrate() throws Exception {
    ...
    configuration.put(
        CONF_SUPPORTED_IMAGE_EXTENSIONS, StringUtils.join(imgExts, CONF_LIST_SEPARATOR));
    // extensions
    ...
    // transtypes
    final String transtypes =
        featureTable.entrySet().stream()
    ...
    configuration.put(
        CONF_PRINT_TRANSTYPES, StringUtils.join(printTranstypes, CONF_LIST_SEPARATOR));
    ...
    for (final Entry<String, Features> e : pluginTable.entrySet()) {
      ...
      if (Boolean.parseBoolean(
          baseDirValues == null || baseDirValues.isEmpty() ? null : baseDirValues.get(0))) {
        ...
        configuration.put(
            ...
            FileUtils.getRelativeUnixPath(
                new File(ditaDir, "dummy").getAbsolutePath(), f.getPluginDir().getAbsolutePath()));
      }
    }
    ...
    final File messagesFile =
        ditaDir.toPath().resolve(CONFIG_DIR).resolve("messages_en_US.properties").toFile();
    ...
    final Collection<File> jars =
        featureTable.containsKey(FEAT_LIB_EXTENSIONS)
  ...
  private Properties readMessageBundle() throws IOException, XMLStreamException {
    ...
    // basePluginDir.resolve(CONFIG_DIR).resolve("messages.xml").toFile();
    final File messagesXmlFile =
    ...
    if (messagesXmlFile.exists()) {
      try (final InputStream in = new FileInputStream(messagesXmlFile)) {
        final XMLStreamReader src =
            XMLInputFactory.newInstance().createXMLStreamReader(new StreamSource(in));
        String id = null;
  ...
  private void customIntegration() {
    final ServiceLoader<CustomIntegrator> customIntegrators =
        ServiceLoader.load(CustomIntegrator.class);
    for (final CustomIntegrator customIntegrator : customIntegrators) {
      ...
      try {
        ...
        logger.error(
            "Custom integrator "
                + customIntegrator.getClass().getName()
                + " failed: "
                + e.getMessage(),
            e);
      }
    ...
    if (checkPlugin(plugin)) {
      ...
      for (final Map.Entry<String, List<String>> currentFeature : featureSet.entrySet()) {
        ...
        final List<Value> values =
            currentFeature.getValue().stream()
        ...
        if (featureTable.containsKey(key)) {
          ...
          // Make shallow clone to avoid making modifications directly to list inside the current
          // feature.
          List<Value> currentFeatureValue = values;
          featureTable.put(
              key, currentFeatureValue != null ? new ArrayList<>(currentFeatureValue) : null);
        }
      }

      for (final Value templateName : pluginFeatures.getAllTemplates()) {
        final String template =
            new File(pluginFeatures.getPluginDir().toURI().resolve(templateName.value()))
                .getAbsolutePath();
  ...
  private boolean checkPlugin(final String currentPlugin) {
    ...
    while (iter.hasNext()) {
      ...
      if (!anyPluginFound && requirement.getRequired()) {
        ...
        final String msg =
            MessageUtils.getMessage("DOTJ020W", requirement.toString(), currentPlugin).toString();
        throw new RuntimeException(msg);
      }
    }
    return true;
   ...
   *
   * <p>Follow OSGi symbolic name syntax rules:
   *
   * <pre>
   * digit         ::= [0..9]
   * alpha         ::= [a..zA..Z]
