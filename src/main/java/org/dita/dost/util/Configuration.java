...
public final class Configuration {
   ...
   *
   * <p>If configuration file is not found e.g. during integration, the configuration will be an
   * empty.
   */
    ...
    try {
      ...
      plugingConfigurationInputStream =
          loader.getResourceAsStream(
              Integrator.class.getPackage().getName() + "/" + GEN_CONF_PROPERTIES);
  ...
  static {
    ...
    for (final Map.Entry<String, String> e : configuration.entrySet()) {
      final String key = e.getKey();
      if (key.startsWith(CONF_PARSER_FORMAT)
          && key.indexOf('.', CONF_PARSER_FORMAT.length()) == -1) {
