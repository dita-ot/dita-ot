...
public final class URLUtils {
   ...
   *
   * @param file The file to be corrected. If null will throw MalformedURLException.
   ...
   *
   * <p>Convenience method for {@link java.net.URLDecoder#decode(String, String) URLDecoder}: use
   * UTF-8 and do not throw {@link java.io.UnsupportedEncodingException
   ...
   * @return The canonical or absolute file, or null if the protocol is not file.
   */
   ...
   *     part.
   * @param forceCorrection True if the correction must be executed any way (for files containing %
   *     for example - the % will be also corrected). Also if <code>true</code> '#' and '?' will be
   *     corrected otherwise will consider that is an URL that contains an anchor or a query.
  ...
  /**
   * Covert file reference to URI. The difference between this method and {@link
   * java.io.File#File(URI)})} constructor is that this method doesn't make the URI absolute.
   *
  ...
  public static URI toURI(final File file) {
    ...
    if (file.isAbsolute()) {
      ...
      try {
        return new URI(
            clean(file.getPath().replace(WINDOWS_SEPARATOR, URI_SEPARATOR).trim(), false));
    ...
    if (base.getPath().equals(ref.getPath()) && ref.getFragment() != null) {
      ...
      while (baseTokenizer.countTokens() > 1 && refTokenizer.countTokens() > 1) {
        ...
        final boolean equals =
            OS_NAME.toLowerCase().contains(OS_NAME_WINDOWS)
   ...
   *
   * <p>For {@code foo/bar/baz.txt} return {@code ../../}
   *
