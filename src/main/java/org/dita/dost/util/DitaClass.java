...
public final class DitaClass {
  ...
  /**
   * ModuleElem/type pair for the most specialized type, with a single preceding and following space
   * character.
   */
   ...
   * @return {@code true} if given attribute set has a class attribute and it matches this class,
   ...
   * @return {@code true} if given node is an Element and its class matches this class, otherwise
   *     {@code false}
   */
   ...
   *
   * @return predicate that returns {@code true} if given node is an Element and its class matches
   *     this class, otherwise {@code false}
   */
  public Predicate<XdmNode> matcher() {
    return item ->
        item.getNodeKind() == XdmNodeKind.ELEMENT && matches(item.attribute(ATTRIBUTE_NAME_CLASS));
  }
