...
public final class Job {
  ...
  private static final String PROPERTY_GENERATE_COPY_OUTER =
      ANT_INVOKER_EXT_PARAM_GENERATECOPYOUTTER;
  private static final String PROPERTY_OUTPUT_DIR = ANT_INVOKER_EXT_PARAM_OUTPUTDIR;
  /** Deprecated since 2.2 */
  @Deprecated private static final String PROPERTY_INPUT_MAP = "InputMapDir";
   ...
   *
   * @return {@code true} if configuration file has been update after this object has been created
   *     or serialized
   */
  ...
  /**
   * Read temporary configuration files. If configuration files are not found, assume an empty job
    ...
    try (Writer outStream =
        new BufferedWriter(
            new OutputStreamWriter(
                getStore().getOutputStream(jobFile.toURI()), StandardCharsets.UTF_8))) {
   ...
   *
   * @return map of file info objects, where the key is the {@link FileInfo#file} value. May be
   *     empty
   */
  ...
  public boolean crawlTopics() {
    ...
    return prop.get(PROPERTY_LINK_CRAWLER)
        .toString()
        .equals(ANT_INVOKER_EXT_PARAM_CRAWL_VALUE_TOPIC);
  }
  ...
  public URI getInputFile() {
    ...
    return files.values().stream()
        ...
        .orElseGet(
            () ->
                Optional.ofNullable((String) prop.get(PROPERTY_INPUT_MAP_URI))
                    .map(URLUtils::toURI)
                    .orElse(null));
  }
  ...
  public TempFileNameScheme getTempFileNameScheme() {
    ...
    try {
      final String cls =
          Optional.ofNullable(getProperty("temp-file-name-scheme"))
