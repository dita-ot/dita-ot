...
public final class FilterUtils {
  ...
  public FilterUtils(
    ...
    dfm.put(
        new FilterKey(QName.valueOf(ATTRIBUTE_NAME_PRINT), ATTR_PRINT_VALUE_YES), Action.INCLUDE);
    if (isPrintType) {
      dfm.put(
          new FilterKey(QName.valueOf(ATTRIBUTE_NAME_PRINT), ATTR_PRINT_VALUE_PRINT_ONLY),
          Action.INCLUDE);
      dfm.put(
          new FilterKey(QName.valueOf(ATTRIBUTE_NAME_PRINT), ATTR_PRINT_VALUE_NO), Action.EXCLUDE);
    } else {
      dfm.put(
          new FilterKey(QName.valueOf(ATTRIBUTE_NAME_PRINT), ATTR_PRINT_VALUE_PRINT_ONLY),
          Action.EXCLUDE);
      dfm.put(
          new FilterKey(QName.valueOf(ATTRIBUTE_NAME_PRINT), ATTR_PRINT_VALUE_NO), Action.INCLUDE);
    }
  ...
  private static Set<QName> getProfileAttributes(final String conf) {
    final ImmutableSet.Builder<QName> res =
        ImmutableSet.<QName>builder()
            .add(
    ...
    if (extProps != null && extProps.length != 0) {
      for (final QName[] propList : extProps) {
        ...
        while ((propValue == null || propValue.trim().isEmpty()) && propListIndex > 0) {
          ...
          propValue =
              getLabelValue(
                  propName, atts.getValue(current.getNamespaceURI(), current.getLocalPart()));
        }
        if (propValue != null) {
  ...
  public boolean needExclude(final Attributes atts, final QName[][] extProps) {
    ...
    if (extProps != null && extProps.length != 0) {
      for (final QName[] propList : extProps) {
        ...
        if (propValue != null
            && extCheckExclude(propList, Arrays.asList(propValue.split("\\s+")))) {
          return true;
        }
      }
    }
  ...
  private String getLabelValue(final QName propName, final String attrPropsValue) {
    if (attrPropsValue != null) {
      int propStart = -1;
      if (attrPropsValue.startsWith(propName + "(")
          || attrPropsValue.contains(" " + propName + "(")) {
  ...
  private FilterUtils refine(final Map<QName, Map<String, Set<Element>>> bindingMap) {
    if (bindingMap != null && !bindingMap.isEmpty()) {
      ...
      final FilterUtils filterUtils =
          new FilterUtils(buf, foregroundConflictColor, backgroundConflictColor);
  ...
  public record Flag(
    ...
    private void writeProp(final ContentHandler contentHandler, final boolean isStart)
        throws SAXException {
      final XMLUtils.AttributesBuilder propAtts =
          new XMLUtils.AttributesBuilder().add("action", "flag");
      if (color != null) {
    ...
    public record FlagImage(URI href, String alt) {
      ...
      private void writeFlag(final ContentHandler contentHandler, final String tag)
          throws SAXException {
        final XMLUtils.AttributesBuilder propAtts =
            new XMLUtils.AttributesBuilder().add("action", "flag");
        final URI abs = href;
        ...
        if (alt != null) {
          contentHandler.startElement(
              NULL_NS_URI, "alt-text", "alt-text", XMLUtils.EMPTY_ATTRIBUTES);
