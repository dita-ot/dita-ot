...
public final class StringUtils {
  ...
  /**
   * Replaces each substring of this string that matches the given string with the given
   * replacement. Differ from the JDK String.replaceAll function, this method does not support
   * regular expression based replacement on purpose.
   *
   * @param input input string
   * @param pattern This pattern is recognized as it is. It will not solve as an regular expression.
  ...
  public static QName[][] getExtPropsFromSpecializations(final String specializations) {
    ...
    return Arrays.stream(specializations.trim().split("\\s+"))
        .map(
            token ->
                Arrays.stream(token.substring(1).split("/"))
                    .map(QName::valueOf)
  ...
  /**
   * If target is null, return the value; else append value to target. If withSpace is true, insert
  ...
  public static String setOrAppend(
      final String target, final String value, final boolean withSpace) {
    if (target == null) {
      return value;
    }
    if (value == null) {
  ...
  public static Locale getLocale(final String anEncoding) {
    ...
    // We need to know how many tokens we have so we can create a Locale object with the proper
    // constructor.
