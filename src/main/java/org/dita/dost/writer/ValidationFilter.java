...
public final class ValidationFilter extends AbstractXMLFilter {
   ...
   *
   * <p>The contents of the map is in pseudo-code {@code Map<AttName, Map<ElemName, <Set<Value>>>}.
   ...
   */
  private AttributesImpl validateLang(final Attributes atts, final AttributesImpl modified)
      throws SAXException {
      ...
      if (i != -1) {
        if (processingMode == Mode.STRICT) {
          throw new SAXException(
              MessageUtils.getMessage("DOTJ056E", lang).setLocation(locator).toString());
        }
  ...
  private AttributesImpl validateId(
      final String localName, final Attributes atts, final AttributesImpl modified)
      throws SAXException {
    AttributesImpl res = modified;
    ...
    if (MAP_MAP.matches(cls)) {
      ...
      if (id == null) {
        switch (processingMode) {
          ...
          case LAX -> {
            ...
            logger.error(
                MessageUtils.getMessage("DOTJ066E", localName, gen)
                    .setLocation(locator)
                    .toString());
          }
      ...
      if (id != null) {
        if (topicIds.contains(id)) {
          if (processingMode == Mode.STRICT) {
            throw new SAXException(
                MessageUtils.getMessage("DOTJ057E", id).setLocation(locator).toString());
          } else {
      ...
      try {
        ...
        if (Objects.equals(abs.getScheme(), "file")) {
          ...
          try {
            ...
            logger.debug(
                String.format("Failed to resolve real path for %s: %s", p, e.getMessage()), e);
          }
        }
  ...
  private AttributesImpl validateScope(final Attributes atts, final AttributesImpl modified) {
    ...
    if (scope == null && href != null && href.isAbsolute()) {
      ...
      final boolean sameAuthority =
          Objects.equals(currentFile.getRawAuthority(), href.getRawAuthority());
      if (!(sameScheme && sameAuthority)) {
        switch (processingMode) {
          case LAX -> {
            ...
            logger.warn(
                MessageUtils.getMessage("DOTJ075W", href.toString())
                    .setLocation(locator)
                    .toString());
          }
          default -> logger.warn(
  ...
  /**
   * Validate attribute generalization. A single element may not contain both generalized and
   * specialized values for the same attribute.
   *
   * @param atts attributes
   * @see <a
   *     href="http://docs.oasis-open.org/dita/v1.2/os/spec/archSpec/attributegeneralize.html">DITA
   *     1.2 specification</a>
   */
          ...
          if (atts.getValue(spec[i].getNamespaceURI(), spec[i].getLocalPart()) != null) {
            for (int j = i - 1; j > -1; j--) {
              if (atts.getValue(spec[j].getNamespaceURI(), spec[j].getLocalPart()) != null) {
                logger.error(
                    MessageUtils.getMessage("DOTJ058E", spec[j].toString(), spec[i].toString())
                        .toString());
              }
            }
          }
   ...
   * @return modified attributes, {@code null} if there have been no changes
   */
  private AttributesImpl processFormatDitamap(
      final Attributes atts, final AttributesImpl modified) {
    if (job == null) {
      return modified;
    }
    AttributesImpl res = modified;
