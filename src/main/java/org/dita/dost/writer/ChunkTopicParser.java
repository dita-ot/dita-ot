...
/**
 * Combine topic into a single file for {@code to-content} chunking. Not reusable and not
 * thread-safe.
 */
public final class ChunkTopicParser extends AbstractChunkTopicParser {
  ...
  public ChunkTopicParser() {
    ...
    //            throw new RuntimeException("Failed to initialize XML parser: " + e.getMessage(),
    // e);
    //        }
  }
  ...
  private void insertAfter(
      final URI hrefValue, final StringBuffer parentResult, final CharSequence tmpContent) {
  ...
  private void writeToContentChunk(
      final String tmpContent, final URI outputFileName, final boolean needWriteDitaTag)
      throws IOException {
    assert outputFileName.isAbsolute();
    logger.info("Writing " + outputFileName);
    try (Writer ditaFileOutput =
        new OutputStreamWriter(
            job.getStore().getOutputStream(outputFileName), StandardCharsets.UTF_8)) {
      if (outputFileName.equals(changeTable.get(outputFileName))) {
        ...
        if (!OS_NAME.toLowerCase().contains(OS_NAME_WINDOWS)) {
          writeProcessingInstruction(
              ditaFileOutput, PI_WORKDIR_TARGET, new File(workDir).getAbsolutePath());
        } else {
          writeProcessingInstruction(
              ditaFileOutput,
              PI_WORKDIR_TARGET,
              UNIX_SEPARATOR + new File(workDir).getAbsolutePath());
        }
        writeProcessingInstruction(ditaFileOutput, PI_WORKDIR_TARGET_URI, workDir.toString());

        final File path2rootmap =
            toFile(getRelativePath(outputFileName, job.getInputMap())).getParentFile();
        writeProcessingInstruction(
            ditaFileOutput,
