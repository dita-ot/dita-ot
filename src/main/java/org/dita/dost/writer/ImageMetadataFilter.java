...
public final class ImageMetadataFilter extends AbstractXMLFilter {
  ...
  public ImageMetadataFilter(
      final File outputDir, final Job job, final Map<URI, Attributes> cache) {
    ...
    public Attributes getAttributes() {
      ...
      if (width != null) {
        a.add(
            DITA_OT_NS,
            ATTR_IMAGE_WIDTH,
            DITA_OT_NS_PREFIX + ":" + ATTR_IMAGE_WIDTH,
            "CDATA",
            width);
      }
      if (height != null) {
        a.add(
            DITA_OT_NS,
            ATTR_IMAGE_HEIGHT,
            DITA_OT_NS_PREFIX + ":" + ATTR_IMAGE_HEIGHT,
            "CDATA",
            height);
      }
      ...
      if (verticalDpi != null) {
        a.add(
            DITA_OT_NS,
            ATTR_VERTICAL_DPI,
            DITA_OT_NS_PREFIX + ":" + ATTR_VERTICAL_DPI,
            "CDATA",
            verticalDpi);
      }
      return a.build();
    ...
    if (imgInput.getScheme().equals("data")) {
      ...
      if (metadata.endsWith(";base64")) {
        ...
        return new ByteArrayInputStream(
            BaseEncoding.base64().decode(data.substring(separator + 1)));
      } else {
