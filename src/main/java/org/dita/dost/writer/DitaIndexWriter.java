...
public final class DitaIndexWriter extends AbstractXMLWriter {
  ...
  public void endElement(final String uri, final String localName, final String qName)
    ...
    try {
      if (!hasMetadataTillNow
          && TOPIC_PROLOG.localName.equals(qName)
          && startTopic
          && !hasWritten) {
  ...
  public void startElement(
    ...
    try {
      ...
      if (atts.getValue(ATTRIBUTE_NAME_CLASS) != null) {
        ...
        if (atts.getValue(ATTRIBUTE_NAME_CLASS).contains(TOPIC_METADATA.matcher)
            && startTopic
            && !hasWritten) {
          hasMetadataTillNow = true;
          output.write(indexEntries);
    ...
    try {
      ...
      output =
          new OutputStreamWriter(
              job.getStore().getOutputStream(outputFile.toURI()), StandardCharsets.UTF_8);
