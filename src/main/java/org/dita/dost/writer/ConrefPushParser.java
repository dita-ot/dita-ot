...
/** This class is for writing conref push contents into specific files. */
public final class ConrefPushParser extends AbstractDomFilter {
  ...
  /**
   * The function is to judge if the pushed content type march the type of content being
   * pushed/replaced
   *
   ...
   */
  private boolean isPushedTypeMatch(
      final DitaClass targetClassAttribute, final DocumentFragment content) {
  ...
  private DocumentFragment replaceElementName(
      final DitaClass targetClassAttribute, final DocumentFragment content) {
    if (content.hasChildNodes()) {
      final NodeList nodeList = content.getChildNodes();
      for (int i = 0; i < nodeList.getLength(); i++) {
  ...
  private void replaceSubElementName(final String type, final Element elem) {
    ...
    if (cls != null) {
      if (cls.toString().contains(type) && !type.equals(STRING_BLANK)) {
        ...
        generalizedElemName =
            cls.toString().substring(index + 1, cls.toString().indexOf(STRING_BLANK, index)).trim();
      }
    }
  ...
  private void walk(final Element elem, final String parentTopicId) {
    ...
    if (!idValue.isEmpty()) {
      ...
      if (TOPIC_TOPIC.matches(cls)) {
        ...
        final MoveKey pushBefore =
            hasAction(cls, idPath, defaultidPath, ATTR_CONACTION_VALUE_PUSHBEFORE);
        if (pushBefore != null) {
          final DocumentFragment fragment = movetable.remove(pushBefore);
          insertBefore(elem, replaceElementName(cls, fragment));
        }

        final MoveKey pushAfter =
            hasAction(cls, idPath, defaultidPath, ATTR_CONACTION_VALUE_PUSHAFTER);
        if (pushAfter != null) {
          final DocumentFragment fragment = movetable.remove(pushAfter);
          insertAfter(elem, replaceElementName(cls, fragment));
        }

        final MoveKey pushReplace =
            hasAction(cls, idPath, defaultidPath, ATTR_CONACTION_VALUE_PUSHREPLACE);
        if (pushReplace != null) {
