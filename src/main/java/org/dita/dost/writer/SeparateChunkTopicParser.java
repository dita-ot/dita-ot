...
/**
 * Split topic into multiple files for {@code by-topic} chunking. Not reusable and not thread-safe.
 */
    ...
    try {
      ...
      if (parseFilePath.getFragment() != null) {
        ...
        if (chunkValue.contains(CHUNK_SELECT_BRANCH)) {
          ...
          firstTopicID =
              getFirstTopicId(new File(stripFragment(currentFile.resolve(parseFilePath))));
      ...
      if (TOPIC_TOPIC.matches(cls)) {
        ...
        if (include && !CHUNK_SELECT_TOPIC.equals(selectMethod)) {
          ...
          if (atts.getIndex(ATTRIBUTE_NAME_XML_LANG) < 0 && currentLang != null) {
            attsMod.addAttribute(
                "", ATTRIBUTE_NAME_LANG, ATTRIBUTE_NAME_XML_LANG, "CDATA", currentLang);
          }
          //                    final FileInfo fi = generateFileInfo(outputFile);
          //                    job.add(fi);
          ...
          if (!OS_NAME.toLowerCase().contains(OS_NAME_WINDOWS)) {
            writeProcessingInstruction(
                output, PI_WORKDIR_TARGET, new File(currentFile).getParentFile().getAbsolutePath());
          } else {
            writeProcessingInstruction(
                output, PI_WORKDIR_TARGET, UNIX_SEPARATOR + currentFile.resolve("."));
          }
          writeProcessingInstruction(
              output, PI_WORKDIR_TARGET_URI, currentFile.resolve(".").toString());

          // create a new child element in separate case topicref is equals to parameter
          // element in separateChunk(Element element)
          final Element newTopicref =
              rootTopicref.getOwnerDocument().createElement(MAP_TOPICREF.localName);
