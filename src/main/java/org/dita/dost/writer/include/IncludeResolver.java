 ...
 *
 * <p>The format attribute is assumed to follow the syntax:
 *
 * <pre>format (";" space* "charset=" charset)?</pre>
 *
 * <p>If no charset if defined or the charset name is not recognized, {@link
 * java.nio.charset.Charset#defaultCharset() default charset} is used in reading the code file.
 *
 * <p>The href attribute can contain an optional line range:
 *
 * <pre>uri ("#line-range(" start ("," end)? ")" )?</pre>
 *
 * <p>Start and end line numbers start from 1 and are inclusive. If end range is omitted, range ends
 * in last line.
 *
 * <p>Optional id range is defined using:
 *
 * <pre>uri ("#token=" start? ("," end)? )?</pre>
 *
 * <p>Lines idenfified using start and end IDs are exclusive. If the start range is omitted, range
 * starts from the first line; if end range is omitted, range ends in last line.
 *
 * @since 3.5
 */
      ...
      if (stackItem.include) {
        try {
          ...
          if (hrefValue != null) {
            ...
            switch (parse) {
              case "text" -> include =
                  new IncludeText(job, currentFile, getContentHandler(), logger, processingMode)
                      .include(atts);
              case "xml" -> include =
