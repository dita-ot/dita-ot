...
public final class NormalizeCodeblock extends AbstractXMLFilter {
  ...
  private final Set<String> outputClass =
      new HashSet<>(Collections.singletonList("normalize-space"));
  private int depth = 0;
  ...
  public void startElement(String uri, String localName, String qName, Attributes atts)
    ...
    if (depth > 0) {
      ...
      buf.add(new StartElementEvent(uri, localName, qName, atts));
        ...
        && hasStripWhitespace(atts.getValue(ATTRIBUTE_NAME_OUTPUTCLASS))) {
      depth = 1;
  ...
  private Collection<SaxEvent> normalizeSpace(Collection<SaxEvent> buf) {
    ...
    for (final SaxEvent event : buf) {
      if (event instanceof final CharactersEvent e) {
        final char[] ch =
            stripLeadingSpace(previousEndedInLinefeed, min, e.ch(), e.start(), e.length());
  ...
  private char[] stripLeadingSpace(boolean first, int prefix, char[] ch, int start, int length) {
    final String str =
        first ? new String(ch, start + prefix, length - prefix) : new String(ch, start, length);
