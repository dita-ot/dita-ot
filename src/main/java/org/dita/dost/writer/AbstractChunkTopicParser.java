...
/**
 * ChunkTopicParser class, writing chunking content into relative topic files and then update list.
...
public abstract class AbstractChunkTopicParser extends AbstractXMLWriter {
  ...
  @Override
  public abstract void endElement(final String uri, final String localName, final String qName)
      throws SAXException;
  ...
  void processSelect(String id) {
    if (include) {
      if (CHUNK_SELECT_TOPIC.equals(selectMethod)) {
        // if select method is "select-topic" and current topic is the nested topic in target topic,
        // skip it.
        include = false;
        skipLevel = 1;
        skip = true;
  ...
  FileInfo generateFileInfo(final URI output) {
    ...
    final FileInfo fi = b.uri(temp).result(result).format(ATTR_FORMAT_VALUE_DITA).build();
    return fi;
  }
  ...
  URI generateOutputFilename(final String id) {
    ...
    if (id == null
        || job.getStore().exists(job.tempDirURI.resolve(temp))) { // job.getFileInfo(result) != null
      final URI t = temp;
  ...
  Attributes processAttributes(final Attributes atts) {
    ...
    if (TOPIC_TOPIC.matches(cls)
        && resAtts.getValue(ATTRIBUTE_NAMESPACE_PREFIX_DITAARCHVERSION) == null) {
      addOrSetAttribute(resAtts, ATTRIBUTE_NAMESPACE_PREFIX_DITAARCHVERSION, DITA_NAMESPACE);
    }
    if (TOPIC_TOPIC.matches(cls)
        && resAtts.getValue(XMLNS_ATTRIBUTE + ":" + DITA_OT_NS_PREFIX) == null) {
    ...
    if (TOPIC_TOPIC.matches(cls)
        && resAtts.getValue(ATTRIBUTE_NAME_NONAMESPACESCHEMALOCATION) != null) {
  ...
  Attributes processAttributesNS(Attributes atts, String uri) {
    ...
    // Check to see we are at the root element to be processed is the start of the namespaced
    // element
   ...
   * @return The first topic id from the given dita file if success, otherwise {@code null} string
   *     is returned.
   */
   ...
   */
  void writeStartElement(final Writer output, final String name, final Attributes atts)
      throws SAXException {
    try {
