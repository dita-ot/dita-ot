...
public abstract class AbstractDitaMetaWriter extends AbstractDomFilter {
  ...
  private static final Set<DitaClass> uniqueSet =
  ...
  void processMetadata(final Element metadataContainer, final List<DitaClass> order) {
    for (int i = 0; i < order.size(); i++) {
      ...
      if (!newChildren.isEmpty()) {
        final Element insertPoint =
            getInsertionRef(metadataContainer, order.subList(i, order.size()));
  ...
  boolean skipUnlockedNavtitle(final Element metadataContainer, final Element checkForNavtitle) {
    if (!TOPIC_TITLEALTS.matches(metadataContainer) || !TOPIC_NAVTITLE.matches(checkForNavtitle)) {
      ...
      return false;
    } else if (ATTRIBUTE_NAME_LOCKTITLE_VALUE_YES.matches(
  ...
  /**
   * Get metadata elements to add to current document. Elements have been cloned and imported into
  ...
  Element findMetadataContainer(
      final Element root, List<DitaClass> position, final DitaClass container) {
