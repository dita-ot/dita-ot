 ...
 *
 * <p>The following processing instructions are added before the root element:
 *
 * <dl>
 *   <dt>{@link Constants#PI_WORKDIR_TARGET}
 *   <dd>Absolute system path of the file parent directory. On Windows, a {@code /} is added to
 *       beginning of the path.
 *   <dt>{@link Constants#PI_WORKDIR_TARGET_URI}
 *   <dd>Absolute URI of the file parent directory.
 *   <dt>{@link Constants#PI_PATH2PROJ_TARGET}
 *   <dd>Relative system path to the output directory, with a trailing directory separator. When the
 *       source file is in the project root directory, processing instruction has no value.
 *   <dt>{@link Constants#PI_PATH2PROJ_TARGET_URI}
 *   <dd>Relative URI to the output directory, with a trailing path separator. When the source file
 *       is in the project root directory, processing instruction has value {@code ./}.
 *   <dt>{@link Constants#PI_PATH2ROOTMAP_TARGET_URI}
 *   <dd>Relative URI to the root map directory, with a trailing path separator. When the source
 *       file is in the project root directory, processing instruction has value {@code ./}.
 * </dl>
 *
...
public final class DitaWriterFilter extends AbstractXMLFilter {
  ...
  public void startDocument() throws SAXException {
    ...
    final File path2Project =
        DebugAndFilterModule.getPathtoProject(
    ...
    if (!OS_NAME.toLowerCase().contains(OS_NAME_WINDOWS)) {
      getContentHandler()
          .processingInstruction(PI_WORKDIR_TARGET, outputFile.getParentFile().getAbsolutePath());
    } else {
      getContentHandler()
          .processingInstruction(
              PI_WORKDIR_TARGET, UNIX_SEPARATOR + outputFile.getParentFile().getAbsolutePath());
    }
    getContentHandler().ignorableWhitespace(new char[] {'\n'}, 0, 1);
    getContentHandler()
        .processingInstruction(PI_WORKDIR_TARGET_URI, outputFile.toURI().resolve(".").toString());
    getContentHandler().ignorableWhitespace(new char[] {'\n'}, 0, 1);
    if (path2Project != null) {
      getContentHandler()
          .processingInstruction(PI_PATH2PROJ_TARGET, path2Project.getPath() + File.separator);
      getContentHandler()
          .processingInstruction(
  ...
  private void processAttributes(
      final String qName, final Attributes atts, final AttributesImpl res) {
  ...
  private String getAttributeValue(
      final String elemQName, final QName attQName, final String value) {
    ...
    if (attValue != null) {
      ...
      if (attValue.toString().length() != 0) {
        ...
        if (f != null) {
          ...
          final URI targetTemp =
              job.tempDirURI.resolve(tempFileNameScheme.generateTempFileName(current));
