...
/**
 * Filter for processing key reference elements in DITA files. Instances are reusable but not
 * thread-safe.
 */
public final class KeyrefPaser extends AbstractXMLFilter {

  /**
   * Set of attributes which should not be copied from key definition to key reference which is
   * {@code <topicref>}.
   */
  ...
  /**
   * Set of attributes which should not be copied from key definition to key reference which is not
   * {@code <topicref>}.
   */
  ...
  /** Stack used to store the place of current element relative to the key reference element. */
  private final Deque<Integer> keyrefLevalStack;

  /**
   * Used to store the place of current element relative to the key reference element. If it is out
   * of range of key reference element it is zero, otherwise it is positive number. It is also used
   * to indicate whether current element is descendant of the key reference element.
   */
  private int keyrefLevel;

  /**
   * Indicates whether the keyref is valid. The descendant element should know whether keyref is
   * valid because keyrefs can be nested.
   */
  private final Deque<Boolean> validKeyref;

  /**
   * Flag indicating whether the key reference element is empty, if it is empty, it should pull
   * matching content from the key definition.
   */
    ...
    if (keyrefLevel != 0 && empty) {
      ...
      if (!validKeyref.isEmpty() && validKeyref.peek()) {
        ...
        if (elemName.peek().equals(MAP_TOPICREF.localName)) {
          final Optional<XdmNode> topicmetaNode =
              elem.select(child().where(MAP_TOPICMETA::matches).first()).findFirst();
          ...
          if (!hasSubElem.peek() && currentElement != null) {
            final List<XdmNode> keywords =
                elem.select(descendant().where(hasLocalName(TOPIC_KEYWORD.localName))).asList();
            final List<XdmNode> keywordsInKeywords =
                keywords.stream()
            ...
            if (!keywordsInKeywords.isEmpty()) {
              if (!currentElement.hasNestedElements) {
                ...
                if (TOPIC_LINK.matches(currentElement.type)) {
                  ...
                  getContentHandler()
                      .startElement(
                          NULL_NS_URI, TOPIC_LINKTEXT.localName, TOPIC_LINKTEXT.localName, atts);
                } else if (TOPIC_IMAGE.matches(currentElement.type)) {
                  final AttributesImpl atts = new AttributesImpl();
                  XMLUtils.addOrSetAttribute(atts, ATTRIBUTE_NAME_CLASS, TOPIC_ALT.toString());
                  getContentHandler()
                      .startElement(NULL_NS_URI, TOPIC_ALT.localName, TOPIC_ALT.localName, atts);
                }
                if (!currentElement.isEmpty) {
                ...
                if (TOPIC_LINK.matches(currentElement.type)) {
                  getContentHandler()
                      .endElement(NULL_NS_URI, TOPIC_LINKTEXT.localName, TOPIC_LINKTEXT.localName);
                } else if (TOPIC_IMAGE.matches(currentElement.type)) {
                  getContentHandler()
                      .endElement(NULL_NS_URI, TOPIC_ALT.localName, TOPIC_ALT.localName);
                }
              }
            } else {
              if (TOPIC_LINK.matches(currentElement.type)) {
                ...
                final List<XdmNode> linktext =
                    elem.select(descendant().where(hasLocalName(TOPIC_LINKTEXT.localName)).first())
                        .asList();
                if (!linktext.isEmpty()) {
                  domToSax(linktext.get(0), true);
                } else if (fallbackToNavtitleOrHref(elem)) {
                  final List<XdmNode> navtitleElement =
                      elem.select(
                              descendant().where(hasLocalName(TOPIC_NAVTITLE.localName)).first())
                          .asList();
  ...
  public void startElement(
      ...
      throws SAXException {
    final KeyScope childScope =
        Optional.ofNullable(atts.getValue(ATTRIBUTE_NAME_KEYSCOPE))
    ...
    if (!hasKeyref(atts) || currentElement == null) {
      // If the keyrefLevel doesn't equal 0, it means that current element is under the key
      // reference element;
      if (keyrefLevel != 0) {
        keyrefLevel++;
        hasSubElem.pop();
          ...
          if (currentElement != null) {
            ...
            if (href != null && !href.toString().isEmpty()) {
              if (TOPIC_IMAGE.matches(currentElement.type)) {
                ...
                final URI relativeTarget =
                    setFragment(
                        URLUtils.getRelativePath(currentFile, topicFile), target.getFragment());
                String topicId = null;
                ...
                if (!ATTR_PROCESSING_ROLE_VALUE_RESOURCE_ONLY.equals(
                    atts.getValue(ATTRIBUTE_NAME_PROCESSING_ROLE))) {
  ...
  /**
   * Serialize DOM node into a SAX stream, while modifying map classes to topic classes for common
   * elements.
   *
   * @param elem element to serialize
   * @param retainElements {@code true} to serialize elements, {@code false} to only serialize text
   *     nodes.
   */
  @VisibleForTesting
   ...
   * @param retainElements {@code true} to serialize elements, {@code false} to only serialize text
   *     nodes.
   * @param swapMapClass {@code true} to change map/ to topic/ in common class attributes, {@code
   *     false} to leave as is
   */
  private void domToSax(
      final XdmNode elem, final boolean retainElements, final boolean swapMapClass)
      throws SAXException {
    ...
    if (retainElements) {
      final AttributesImpl atts = new AttributesImpl();
      elem.select(attribute())
          .forEach(
              a -> {
                if (Objects.equals(a.getNodeName(), new QName(ATTRIBUTE_NAME_CLASS))
                    && swapMapClass) {
                  XMLUtils.addOrSetAttribute(
                      atts, ATTRIBUTE_NAME_CLASS, changeclassValue(a.getStringValue()));
                } else {
                  XMLUtils.addOrSetAttribute(atts, a);
                }
              });
      getContentHandler()
          .startElement(qName.getNamespaceURI(), qName.getLocalName(), qNameString, atts);
    }
  ...
  private static URI normalizeHrefValue(
      final URI fileName, final String tail, final String topicId) {
