...
public final class DitaLinksWriter extends AbstractXMLFilter {
  ...
  private static final Attributes relatedLinksAtts =
      new AttributesBuilder().add(ATTRIBUTE_NAME_CLASS, TOPIC_RELATED_LINKS.toString()).build();
  private URI baseURI;
   ...
   * Set relates links
   *
   * @param indexEntries map of related links. Keys are topic IDs and {@link
   *     org.dita.dost.util.Constants#SHARP #} is used to denote root element
   */
    ...
    if (TOPIC_TOPIC.matches(atts)) {
      ...
      if (curMatchTopic != null && !firstTopic) {
        try {
          ...
          getContentHandler()
              .endElement(
                  NULL_NS_URI, TOPIC_RELATED_LINKS.localName, TOPIC_RELATED_LINKS.localName);
          curMatchTopic = null;
