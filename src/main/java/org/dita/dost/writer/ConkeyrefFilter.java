...
/**
 * Filter for processing content key reference elements in DITA files. Instances are reusable but
 * not thread-safe.
 */
    ...
    if (conkeyref != null) {
      ...
      if (keyDef != null) {
        ...
        if (k.href != null && (k.scope.equals(ATTR_SCOPE_VALUE_LOCAL))) {
          ...
          logger.warn(
              MessageUtils.getMessage("DOTJ060W", key, conkeyref).setLocation(atts).toString());
        }
      } else {
