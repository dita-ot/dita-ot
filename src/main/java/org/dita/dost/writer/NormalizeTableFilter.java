 ...
 *
 * <ul>
 *   <li>Rewrite table column names to {@code "col" num}, where {@code num} is the column number,
 *       and add column name to every entry.
 *   <li>Add column coordinates to entries
 * </ul>
 */
public class NormalizeTableFilter extends AbstractXMLFilter {
  ...
  public void startDocument() throws SAXException {
    ...
    processingMode =
        mode != null ? Configuration.Mode.valueOf(mode.toUpperCase()) : Configuration.Mode.LAX;
  ...
  public void startElement(
    ...
    if (TOPIC_TGROUP.matches(cls)) {
      ...
      if (tableState.previousRow != null) {
        final List<Span> fromPrew =
            tableState.previousRow.stream()
                .map(
                    s -> {
                      if (s == null) {
                        return null;
  ...
  private void generateColSpec() throws SAXException {
    ...
    processColspec(attr);
    getContentHandler()
        .startElement(NULL_NS_URI, TOPIC_COLSPEC.localName, TOPIC_COLSPEC.localName, attr);
  ...
  private void processEntry(AttributesImpl res) throws SAXException {
    try {
      ...
      XMLUtils.addOrSetAttribute(
          res, ATTRIBUTE_NAME_COLNAME, COLUMN_NAME_COL + (tableState.currentColumn + 1));
      if (res.getValue(ATTRIBUTE_NAME_NAMEST) != null) {
        XMLUtils.addOrSetAttribute(
            res, ATTRIBUTE_NAME_NAMEST, COLUMN_NAME_COL + tableState.columnNumber);
      }
      if (res.getValue(ATTRIBUTE_NAME_NAMEEND) != null) {
        XMLUtils.addOrSetAttribute(
            res,
            ATTRIBUTE_NAME_NAMEEND,
            COLUMN_NAME_COL + getEndNumber(res, tableState.columnNumber));
        XMLUtils.addOrSetAttribute(
            res,
            DITA_OT_NS,
            ATTR_MORECOLS,
    ...
    try {
      ...
      if (processingMode == Configuration.Mode.STRICT) {
        throw new SAXException(
            MessageUtils.getMessage("DOTJ062E", ATTRIBUTE_NAME_COLS, c)
                .setLocation(atts)
                .toString());
      } else {
        logger.error(
  ...
  private void processColspec(final AttributesImpl res) {
    ...
    final String colName =
        res.getValue(ATTRIBUTE_NAME_COLNAME) != null
    ...
    if (colNum == null || colNum.isEmpty()) {
      XMLUtils.addOrSetAttribute(
          res, ATTRIBUTE_NAME_COLNUM, Integer.toString(tableState.columnNumber));
    }

    tableState.columnNumberEnd = tableState.columnNumber;
    // change the col name of colspec
    XMLUtils.addOrSetAttribute(
        res, ATTRIBUTE_NAME_COLNAME, COLUMN_NAME_COL + tableState.columnNumber);
    // total columns count
