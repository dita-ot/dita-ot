...
public class SubjectSchemeReader {
  ...
  /**
   * Get map of valid attribute values based on subject scheme. The contents of the map is in
   * pseudo-code {@code Map<AttName, Map<ElemName, <Set<Value>>>}. For default element mapping, the
   * value is {@code *}.
   *
  ...
  /**
   * Get map of default values based on subject scheme. The contents of the map is in pseudo-code
   * {@code Map<AttName, Map<ElemName, Default>>}. For default element mapping, the value is {@code
   * *}.
   *
  ...
  /**
   * Get map subject scheme definitions. The contents of the map is in pseudo-code {@code
   * Map<AttName, Map<ElemName, Set<Element>>>}. For default element mapping, the value is {@code
   * *}.
   *
  ...
  /**
   * Read a map from XML properties file. Values are split by {@link
   * org.dita.dost.util.Constants#COMMA COMMA} into a set.
   *
  ...
  public Map<URI, Set<URI>> readMapFromXML(final File inputFile) throws IOException {
    ...
    try (InputStream in =
        new BufferedInputStream(job.getStore().getInputStream(inputFile.toURI()))) {
      prop.loadFromXML(in);
