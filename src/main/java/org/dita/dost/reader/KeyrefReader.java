...
/**
 * KeyrefReader class which reads DITA map file to collect key definitions. Instances are reusable
 * but not thread-safe.
 */
public final class KeyrefReader implements AbstractReader {
  ...
  private void cascadeChildKeys(
      final KeyScope scope, final Map<String, KeyDef> keys, final String prefix) {
    for (final Map.Entry<String, KeyDef> e : scope.keyDefinition().entrySet()) {
      final KeyDef oldKeyDef = e.getValue();
      final KeyDef newKeyDef =
          new KeyDef(
  ...
  private KeyScope resolveIntermediate(final KeyScope scope) {
    ...
    for (final Map.Entry<String, KeyDef> e : scope.keyDefinition().entrySet()) {
      final KeyDef res =
          resolveIntermediate(scope, e.getValue(), Collections.singletonList(e.getValue()));
  ...
  private KeyDef resolveIntermediate(
    ...
    if (keyref != null && !keyref.trim().isEmpty() && scope.keyDefinition().containsKey(keyref)) {
      ...
      return new KeyDef(
          keyDef.keys, keyRefDef.href, keyRefDef.scope, keyRefDef.format, keyRefDef.source, res);
    } else {
      return keyDef;
    }
  }
  ...
  private void handleCircularDefinitionException(final List<KeyDef> circularityTracker) {
    ...
    final MessageBean ex =
        MessageUtils.getMessage("DOTJ069E", sb.toString())
  ...
  private XdmNode mergeMetadata(final XdmNode defElem, final XdmNode refElem) {
    try {
      ...
      final AttributeMap atts =
          new SmallAttributeMap(
              defElem.getUnderlyingNode().attributes().asList().stream()
      ...
      if (defMeta != null) {
        ...
        if (resMeta == null) {
          final SingletonAttributeMap attrs =
              SingletonAttributeMap.of(
                  new AttributeInfo(
