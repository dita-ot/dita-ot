...
/**
 * MergeTopicParser reads topic file and transform the references to other dita files into internal
 * references. The parse result of MergeTopicParser will be returned to MergeMapParser and serves as
 * part of intermediate merged result. Instances are reusable but not thread-safe.
 */
public final class MergeTopicParser extends XMLFilterImpl {
  ...
  private URI handleLocalDita(final URI href, final AttributesImpl atts) {
    ...
    if (sharpIndex != -1) { // href value refer to an id in a topic
      ...
      final URI pathWithTopicID =
          setFragment(URLUtils.toDirURI(dirPath).resolve(pathFromMap), topicID);
    ...
    if (attValue != null) {
      ...
      if (isLocalScope(scopeValue) && attValue.getScheme() == null) {
        ...
        if ((TOPIC_XREF.matches(classValue)
          ...
          XMLUtils.addOrSetAttribute(
              atts, ATTRIBUTE_NAME_HREF, handleLocalDita(attValue, atts).toString());
        } else {
