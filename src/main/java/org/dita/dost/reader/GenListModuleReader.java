 ...
 *
 * <p><strong>Not thread-safe</strong>. Instances can be reused by calling {@link #reset()} between
 * calls to parse.
 */
public final class GenListModuleReader extends AbstractXMLFilter {
  ...
  /**
   * Relationship graph between subject schema. Keys are subject scheme map paths and values are
   * subject scheme map paths, both relative to base directory. A key {@link #ROOT_URI} contains all
   * subject scheme maps.
   */
  ...
  /**
   * Get relationship graph between subject schema. Keys are subject scheme map paths and values are
   * subject scheme map paths, both relative to base directory. A key {@link #ROOT_URI} contains all
   * subject scheme maps.
   *
  ...
  public void startElement(
    ...
    if (href != null
      ...
      if (isFormatDita(atts.getValue(ATTRIBUTE_NAME_FORMAT))
          && !isDitaMap()
          && !job.crawlTopics()) {
    ...
    if (!(cls != null && cls.isValid()) && !ELEMENT_NAME_DITA.equals(localName)) {
      if (nonDitaContext(classes)) {
        ...
        logger.info(
            MessageUtils.getMessage("DOTJ070I", atts.getValue(ATTRIBUTE_NAME_CLASS), localName)
                .setLocation(atts)
                .toString());
      }
    } else {
  ...
  private void handleSubjectScheme(final Attributes atts) {
    ...
    if (SUBJECTSCHEME_SUBJECTSCHEME.matches(classValue)) {
      ...
      final Set<URI> children =
          schemeRelationGraph.containsKey(key)
  ...
  private void parseAttribute(final Attributes atts, final String attrName) throws SAXException {
    ...
    if (attrScope == null && ATTRIBUTE_NAME_HREF.equals(attrName) && attrValue.isAbsolute()) {
      switch (attrValue.getScheme()) {
        case "http", "https", "ftp", "ftps", "sftp", "mailto" -> attrScope =
    ...
    final URI filename =
        stripFragment(attrValue.isAbsolute() ? attrValue : currentDir.resolve(attrValue));
    assert filename.isAbsolute();

    final String attrType = atts.getValue(ATTRIBUTE_NAME_TYPE);
    if (MAP_TOPICREF.matches(attrClass)
        && ATTR_TYPE_VALUE_SUBJECT_SCHEME.equalsIgnoreCase(attrType)) {
          ...
          if (copyTo.toString().isEmpty()) {
            ...
            if (copytoMap.get(filename) != null) {
              if (!value.equals(copytoMap.get(filename))) {
                logger.warn(
                    MessageUtils.getMessage("DOTX065W", copyTo.toString(), filename.toString())
                        .setLocation(atts)
                        .toString());
              }
  ...
  private void parseConactionAttr(final Attributes atts) {
    ...
    if (conaction != null) {
      if (conaction.equals(ATTR_CONACTION_VALUE_MARK)
          || conaction.equals(ATTR_CONACTION_VALUE_PUSHREPLACE)) {
        hasconaction = true;
      }
    }
  }
      ...
      if (isOutFile(filename)) {
        if (job.getOutterControl() == Job.OutterControl.FAIL) {
          ...
          throw new SAXParseException(
              null, null, new DITAOTException(msgBean, null, msgBean.toString()));
