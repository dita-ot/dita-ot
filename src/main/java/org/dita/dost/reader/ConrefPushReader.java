...
public final class ConrefPushReader extends AbstractXMLReader {
  ...
  /**
   * keep the file path of current file under parse filePath is useful to get the absolute path of
   * the target file.
   */
  ...
  /**
   * pushcontent is used to store the content copied to target in pushcontent href will be resolved
   * if it is relative path if @conref is in pushconref the target name should be recorded so that
   * it could be added to conreflist for conref resolution.
   */
  ...
  /**
   * boolean start is used to control whether sax parser can start to record push content into
   * String pushcontent.
   */
  private boolean start = false;
  /**
   * level is used to record the level number to the root element in pushcontent In endElement(...)
   * we can turn start off to terminate adding content to pushcontent if level is zero. That means
   * we reach the end tag of the starting element.
   */
  private int level = 0;

  /**
   * target is used to record the target of the conref push if we reach pushafter action but there
   * is no target recorded before, we need to report error.
   */
  private URI target = null;

  /**
   * pushType is used to record the current type of push it is used in endElement(....) to tell
   * whether it is pushafter or replace.
   */
      ...
      switch (conactValue) {
        case ATTR_CONACTION_VALUE_PUSHBEFORE -> {
          if (pushcontentDocumentFragment.getChildNodes().getLength() != 0) {
            // there are redundant "pushbefore", create a new pushcontent and emit a warning
            // message.
    ...
    try {
      ...
      if (ATTR_CONACTION_VALUE_PUSHREPLACE.equals(pushType)
        ...
        if (fragment == null) {
          ...
          logger.error(
              MessageUtils.getMessage("DOTJ041E", target.toString()).setLocation(atts).toString());
        } else {
          String id;
          // has element id
