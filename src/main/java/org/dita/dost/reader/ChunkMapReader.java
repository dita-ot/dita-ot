...
public final class ChunkMapReader extends AbstractDomFilter {
  ...
  private final ChunkFilenameGenerator chunkFilenameGenerator =
      ChunkFilenameGeneratorFactory.newInstance();
   ...
   *
   * <p>TODO: Instead of reclassing map element to be a topicref, add a topicref into the map root
   * and move all map content into that topicref.
   */
  ...
  private void createTopicStump(final URI newFile) {
    try (final OutputStream newFileWriter = job.getStore().getOutputStream(newFile)) {
      final XMLStreamWriter o =
          XMLOutputFactory.newInstance().createXMLStreamWriter(newFileWriter, UTF8);
  ...
  private void processNavitation(final Element topicref) {
    ...
    final String newMapFile =
        chunkFilenameGenerator.generateFilename("MAPCHUNK", FILE_EXTENSION_DITAMAP);
  ...
  private void generateStumpTopic(final Element topicref) {
    ...
    final FileInfo fi =
        new FileInfo.Builder().uri(temp).result(result).format(ATTR_FORMAT_VALUE_DITA).build();
    job.add(fi);
  }
   ...
   * @return text value
   */
  private String getChildElementValueOfTopicmeta(
      final Element element, final DitaClass classValue) {
  ...
  /**
   * Before combining topics in a branch, ensure any descendant topicref with @chunk and no @href
   * has a stub
   */
  ...
  private void updateReltable(final Element elem) {
    ...
    if (href.length() != 0) {
      if (changeTable.containsKey(currentFile.resolve(href))) {
        URI res =
            getRelativePath(currentFile.resolve(FILE_NAME_STUB_DITAMAP), currentFile.resolve(href));
