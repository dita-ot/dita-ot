...
public final class MapMetaReader extends AbstractDomFilter {
  ...
  private static final List<String> metaPos =
  ...
  private Map<String, Element> cloneElementMap(final Map<String, Element> current) {
    ...
    for (final Entry<String, Element> topicMetaItem : current.entrySet()) {
      topicMetaTable.put(
          topicMetaItem.getKey(), (Element) resultDoc.importNode(topicMetaItem.getValue(), true));
    }
    return topicMetaTable;
  }

  private Map<String, Element> handleMeta(
      final Element meta, final Map<String, Element> inheritance) {
    final Map<String, Element> topicMetaTable = new HashMap<>(16);
      ...
      if (node.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (classAttr != null) {
          ...
          final String metaKey =
              classValue.substring(
                  1, classValue.indexOf(STRING_BLANK, classValue.indexOf(SLASH)) + 1);
          if (TOPIC_METADATA.matches(classValue)) {
            ...
            if (TOPIC_NAVTITLE.matches(classValue)) {
              ...
              final String locktitleAttr =
                  ((Element) meta.getParentNode()).getAttributeNode(ATTRIBUTE_NAME_LOCKTITLE)
                          != null
