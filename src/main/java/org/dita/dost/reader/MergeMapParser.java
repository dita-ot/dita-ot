...
/**
 * MergeMapParser reads the ditamap file after preprocessing and merges different files into one
 * intermediate result. It calls MergeTopicParser to process the topic file. Instances are reusable
 * but not thread-safe.
 */
public final class MergeMapParser extends XMLFilterImpl {
  ...
  public void characters(final char[] ch, final int start, final int length) throws SAXException {
    if (processStack.empty()
        || !ATTR_PROCESSING_ROLE_VALUE_RESOURCE_ONLY.equals(processStack.peek())) {
          ...
          if (util.isVisited(absTarget)) {
            ...
            if (job.getStore().exists(f.toURI())) {
              ...
              if (attValue.getFragment() != null
                  && util.getIdValue(stripFragment(absTarget)) == null) {
              ...
              XMLUtils.addOrSetAttribute(
                  atts, ATTRIBUTE_NAME_FIRST_TOPIC_ID, firstTopicId.toString());
            } else {
              final URI fileName = URLUtils.toDirURI(dirPath).resolve(attValue);
              logger.error(
                  MessageUtils.getMessage("DOTX008E", fileName.toString())
