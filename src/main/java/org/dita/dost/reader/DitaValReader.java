...
/**
 * DitaValReader reads and parses the information from ditaval file which contains the information
 * of filtering and flagging.
 *
...
public final class DitaValReader implements AbstractReader {
  ...
  public void setJob(final Job job) {
    this.job = job;
    // XXX: This is a hack to disable temp filename generation, because in gen-list the base input
    // dir is unknown.
  ...
  /**
   * Set the map of subject scheme definitions. The contents of the map is in pseudo-code {@code
    ...
    Action action =
        switch (attAction) {
          ...
          default -> throw new IllegalArgumentException(
              MessageUtils.getMessage("DOTJ077F", attAction).setLocation(elem).toString());
        };
    if (action != null) {
      final QName attName;
    ...
    if (children.getLength() != 0) {
      ...
      if (!img.getAttribute(ATTRIBUTE_NAME_IMAGEREF).isEmpty()) {
        ...
        if (tempFileNameScheme != null && job.getFileInfo(absolute) == null) {
          ...
          final Job.FileInfo.Builder fi =
              new Job.FileInfo.Builder().src(absolute).uri(dstTemp).format("flag");
          job.add(fi.build());
        }
      }
