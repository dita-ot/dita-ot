...
public final class Processor {
  ...
  public Processor setOutputDir(final File output) {
    ...
    if (output.exists() && !output.isDirectory()) {
      throw new IllegalArgumentException(
          "Output directory exists and is not a directory: " + output);
    }
  ...
  public Processor setOutputDir(final URI output) {
    ...
    if (!output.getScheme().equals("file")) {
      throw new IllegalArgumentException(
          "Only file scheme allowed as output directory URI: " + output);
    }
  ...
  /**
   * Write a debug log to temporary directory. The name of the debug log is temporary file with
   * {@code .log} extension. By default debug log is generated
   *
  ...
  public void run() throws DITAOTException {
    ...
    final ch.qos.logback.classic.Logger debugLogger =
        createDebugLog ? openDebugLogger(tempDir) : null;
