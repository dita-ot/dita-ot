...
public final class PluginInstallTask extends Task {
  ...
  public void init() {
    registries =
        Arrays.stream(Configuration.configuration.get("registry").trim().split("\\s+"))
  ...
  private Set<Registry> readRegistry(final String name, final SemVerMatch version) {
    ...
    for (final String registry : registries) {
      ...
      try (BufferedInputStream in = new BufferedInputStream(registryUrl.toURL().openStream())) {
        ...
        log(
            String.format("Invalid registry URL %s: %s", registryUrl, e.getMessage()),
            e,
            Project.MSG_ERR);
        ...
        log(
            String.format(
                "Failed to read registry configuration %s: %s", registryUrl, e.getMessage()),
            e,
            Project.MSG_ERR);
      }
    }
    if (res == null) {
      throw new BuildException(
          "Unable to find plugin " + pluginFile + " in any configured registry.");
    }
  ...
  private Optional<Registry> findPlugin(
      final Collection<Registry> regs, final SemVerMatch version) {
    if (version == null) {
      return regs.stream()
  ...
  boolean matchingPlatformVersion(final Registry reg) {
    final Optional<Dependency> platformDependency =
        reg.deps.stream().filter(dep -> dep.name.equals("org.dita.base")).findFirst();
