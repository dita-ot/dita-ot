    ...
    if (input.getAttribute(ANT_INVOKER_PARAM_DITAVAL) != null) {
      ...
      for (final String oneDitavalFile : allDitavalFiles) {
        ...
        if (new File(usingDitavalInput).exists()) {
          ...
          logger.error(MessageUtils.getMessage("DOTJ071E", usingDitavalInput.toString()).toString());
        }
      }
    }
  }
      ...
      for (int i = 0; i < conditionElements.getLength(); i++) {
        ...
        switch (node.getNodeType()) {
          case Node.ELEMENT_NODE -> {
            ...
            if (atts.getNamedItem(ATTRIBUTE_NAME_IMAGEREF) != null || atts.getNamedItem(ATTRIBUTE_NAME_IMG) != null) {
              final String imagerefAtt = atts.getNamedItem(ATTRIBUTE_NAME_IMAGEREF) != null
                ...
                : atts.getNamedItem(ATTRIBUTE_NAME_IMG).getNodeValue(); // DITA 1.1 and later: use @imageref on <startflag>, <endflag> // Pre-DITA 1.1: use @img on <prop>
