...
/**
 * This class reads a list of DITAVAL files, and merges conditions into a single file
 *
 * @since 2.5
 * @author robander
 */
public final class MergeDitavalModule extends AbstractPipelineModuleImpl {
  ...
  private void setMergedProperty() {
    job.setProperty(
        "dita.input.valfile", new File(job.tempDir, FILE_NAME_MERGED_DITAVAL).toString());
  }
  ...
  private void writeConditions(
      XMLStreamWriter export, NodeList conditionElements, URI ditavalDirectory) {
    try {
        ...
        switch (node.getNodeType()) {
          case Node.ELEMENT_NODE -> {
            ...
            if (atts.getNamedItem(ATTRIBUTE_NAME_IMAGEREF) != null
              ...
              final String imagerefAtt =
                  atts.getNamedItem(ATTRIBUTE_NAME_IMAGEREF) != null
