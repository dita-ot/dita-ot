...
final class TopicMergeModule extends AbstractPipelineModuleImpl {
  ...
  public AbstractPipelineOutput execute(final AbstractPipelineInput input) throws DITAOTException {
    ...
    final File style =
        input.getAttribute(ANT_INVOKER_EXT_PARAM_STYLE) != null
    ...
    try (final OutputStream output =
        new BufferedOutputStream(job.getStore().getOutputStream(out.toURI()))) {
      if (style != null) {
        ...
        final StreamSource source =
            new StreamSource(new ByteArrayInputStream(midBuffer.toByteArray()));
