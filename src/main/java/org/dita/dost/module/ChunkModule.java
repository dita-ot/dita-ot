 ...
 *
 * <p>Starting from map files, it parses and processes chunk attribute, writes out the chunked
...
public final class ChunkModule extends AbstractPipelineModuleImpl {
  ...
  private static final DitaClass ECLIPSEMAP_PLUGIN =
      DitaClass.getInstance("- map/map eclipsemap/plugin ");
  ...
  private void updateList(
      final Map<URI, URI> changeTable,
      final Map<URI, URI> conflictTable,
      final ChunkMapReader mapReader) {
          ...
          if (!job.getStore().exists(target)) {
            ...
            if (relativeTargetPath.getPath().contains(URI_SEPARATOR)) {
              relativePath2fix.put(
                  ...
                  relativeTargetPath
                      .getPath()
                      .substring(0, relativeTargetPath.getPath().lastIndexOf(URI_SEPARATOR) + 1));
            }
