...
public final class CopyToModule extends AbstractPipelineModuleImpl {
  ...
  private Map<FileInfo, FileInfo> getCopyToMap() {
    ...
    if (forceUnique) {
      forceUniqueFilter.copyToMap.forEach(
          (dstFi, srcFi) -> {
            job.add(dstFi);
  ...
  /**
   * XML filter to rewrite processing instructions to reflect copy-to location. The following
   * processing-instructions are processed:
   *
   * <ul>
   *   <li>{@link Constants#PI_WORKDIR_TARGET PI_WORKDIR_TARGET}
   *   <li>{@link Constants#PI_WORKDIR_TARGET_URI PI_WORKDIR_TARGET_URI}
   *   <li>{@link Constants#PI_PATH2PROJ_TARGET PI_PATH2PROJ_TARGET}
   *   <li>{@link Constants#PI_PATH2PROJ_TARGET_URI PI_PATH2PROJ_TARGET_URI}
   *   <li>{@link Constants#PI_PATH2ROOTMAP_TARGET_URI PI_PATH2ROOTMAP_TARGET_URI}
   * </ul>
   */
    ...
    public void startElement(
      ...
      if ((TOPIC_XREF.matches(atts) || TOPIC_LINK.matches(atts) || TOPIC_IMAGE.matches(atts))
        ...
        if (value != null && !value.startsWith("#")) {
          resAtts =
              new XMLUtils.AttributesBuilder(atts)
