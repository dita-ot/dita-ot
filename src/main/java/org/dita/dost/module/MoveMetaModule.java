 ...
 *
 * <p>MoveMetaModule implement the move meta step in preprocess. It cascades metadata in maps and
 * collects metadata for topics. The collected metadata is then inserted into maps and topics.
 *
...
final class MoveMetaModule extends AbstractPipelineModuleImpl {
  ...
  private void pullTopicMetadata(final AbstractPipelineInput input, final Collection<FileInfo> fis)
    ...
    for (final FileInfo f : fis) {
      ...
      final File tmp =
          new File(
              inputFile.getAbsolutePath() + ".tmp" + Long.toString(System.currentTimeMillis()));
