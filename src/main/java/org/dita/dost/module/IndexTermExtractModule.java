...
/**
 * This class extends AbstractPipelineModule, used to extract indexterm from dita/ditamap files.
 *
...
public class IndexTermExtractModule extends AbstractPipelineModuleImpl {
  ...
  private void extractIndexTerm() throws SAXException {
    ...
    final DitamapIndexTermReader ditamapIndexTermReader =
        new DitamapIndexTermReader(indexTermCollection, true);
    ...
    for (final URI aTopicList : topicList) {
      ...
      targetPathFromMapWithoutExt =
          targetPathFromMap.substring(0, targetPathFromMap.lastIndexOf("."));
    ...
    for (final URI ditamap : ditamapList) {
      ...
      if (currentMapPathName.lastIndexOf(SLASH) != -1) {
        mapPathFromInputMap =
            currentMapPathName.substring(0, currentMapPathName.lastIndexOf(SLASH));
      }
