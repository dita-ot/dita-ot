 ...
 *
 * <p>Branch filtering is done with the following steps:
 *
 * <ol>
 *   <li>Split braches so that each branch will only contain a single ditavalref
 *   <li>Generate copy-to attribute for each brach generated topicref
 *   <li>Filter map based on branch filters
 *   <li>Rewrite duplicate generated copy-to targets
 *   <li>Copy and filter generated copy-to targets
 *   <li>Filter topics that were not branch generated
 * </ol>
 *
 * @since 2.2
 */
public class BranchFilterModule extends AbstractPipelineModuleImpl {
      ...
      if (attrsMaps.size() > 1) {
        ...
        for (final Map.Entry<Set<URI>, List<Attr>> attrsMap : attrsMaps.entrySet()) {
          ...
          for (final Attr attr : attrs) {
            ...
            logger.info(
                MessageUtils.getMessage("DOTJ065I", attr.getValue(), gen)
            ...
            if (dstUri != null) {
              ...
              if (hrefFileInfo != null) {
                ...
                final FileInfo.Builder dstBuilder =
                    new FileInfo.Builder(hrefFileInfo).uri(dstUri).result(newResult);
  ...
  private boolean isDitaFormat(final Attr formatAttr) {
    return formatAttr == null || isDitaFormat(formatAttr.getNodeValue());
  }
  ...
  private void filterBranches(
      final Element elem, final List<FilterUtils> filters, final QName[][] props) {
    ...
    if (exclude) {
      ...
      for (Flag flag : flags) {
        final Element startElement =
            (Element) elem.getOwnerDocument().importNode(flag.getStartFlag(), true);
        ...
        final Element endElement =
            (Element) elem.getOwnerDocument().importNode(flag.getEndFlag(), true);
  ...
  private List<FilterUtils> combineFilterUtils(
      final Element topicref, final List<FilterUtils> filters) {
  ...
  /**
   * Copy and filter topics for branches. These topics have a new name and will be added to job
   * configuration.
   */
    ...
    if (!copyTo.isEmpty()) {
      ...
      if (srcFileInfo != null) {
        ...
        try {
          job.getStore()
              .transform(
                  URLUtils.removeFragment(srcAbsUri), URLUtils.removeFragment(dstAbsUri), pipe);
        } catch (final DITAOTException e) {
          logger.error(
              "Failed to filter " + srcAbsUri + " to " + dstAbsUri + ": " + e.getMessage(), e);
        }
  ...
  private void filterTopics(final Element topicref, final List<FilterUtils> filters) {
    ...
    if (!fs.isEmpty()
        ...
        && !ATTR_PROCESSING_ROLE_VALUE_RESOURCE_ONLY.equals(
            topicref.getAttribute(ATTRIBUTE_NAME_PROCESSING_ROLE))
  ...
  public static class Branch {
    ...
    private Optional<String> getKeyscopePrefix(
        final Element ditavalref, final Optional<String> oldValue) {
      final Optional<String> v = get(ditavalref, DITAVAREF_D_DVR_KEYSCOPEPREFIX);
      return v.map(s -> Optional.of(oldValue.orElse("") + s)).orElse(oldValue);
    }

    private Optional<String> getKeyscopeSuffix(
        final Element ditavalref, final Optional<String> oldValue) {
    ...
    if (filter.resourcePrefix.isPresent() || filter.resourceSuffix.isPresent()) {
      ...
      if ((!href.isEmpty() || !copyTo.isEmpty()) && !scope.equals(ATTR_SCOPE_VALUE_EXTERNAL)) {
        ...
        if (isDitaFormat(hrefFileInfo.format)) {
          final FileInfo copyToFileInfo =
              !copyTo.isEmpty() ? job.getFileInfo(currentFile.resolve(copyTo)) : null;
          final URI dstSource;
          dstSource =
              generateCopyTo(
                  (copyToFileInfo != null ? copyToFileInfo : hrefFileInfo).result, filter);
          final URI dstTemp = tempFileNameScheme.generateTempFileName(dstSource);
          final String dstPathFromMap =
              !copyTo.isEmpty() ? FilenameUtils.getPath(copyTo) : FilenameUtils.getPath(href);
          final FileInfo.Builder dstBuilder =
          ...
          elem.setAttribute(
              BRANCH_COPY_TO, dstPathFromMap + FilenameUtils.getName(dstTemp.toString()));
          if (!copyTo.isEmpty()) {
