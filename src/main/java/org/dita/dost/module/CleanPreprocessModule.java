...
public class CleanPreprocessModule extends AbstractPipelineModuleImpl {
  ...
  private void init(final Map<String, String> input) {
    useResultFilename =
        Optional.ofNullable(input.get(PARAM_USE_RESULT_FILENAME))
            .map(Boolean::parseBoolean)
            .orElse(false);
    final CatalogResolver catalogResolver = CatalogUtils.getCatalogResolver();
    rewriteTransformer =
        Optional.ofNullable(input.get("result.rewrite-rule.xsl"))
            .map(
                file -> {
                  try {
                ...
                f -> {
                  try {
                    ...
                    throw new RuntimeException(
                        "Failed to compile XSLT: " + e.getXPathException().getMessageAndLocation(),
                        e);
            ...
            .orElse(null);
    rewriteClass =
        Optional.ofNullable(input.get("result.rewrite-rule.class"))
            .map(
                c -> {
                  try {
  ...
  public AbstractPipelineOutput execute(final Map<String, String> input) throws DITAOTException {
    ...
    if (useResultFilename) {
      ...
      final Collection<FileInfo> original =
      ...
      for (final FileInfo fi : rewritten) {
        try {
          ...
          final FileInfo res =
              FileInfo.builder(fi).uri(fi.result).result(base.resolve(fi.result)).build();
          job.add(res);
        } catch (final IOException e) {
          logger.error(
              "Failed to clean " + job.tempDirURI.resolve(fi.uri) + ": " + e.getMessage(), e);
        }
      }
    }
  ...
  private List<XMLFilter> getProcessingPipe(
      final FileInfo fi, final File srcFile, final File destFile) {
