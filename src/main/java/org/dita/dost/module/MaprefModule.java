...
final class MaprefModule extends AbstractPipelineModuleImpl {
  ...
  public AbstractPipelineOutput execute(final AbstractPipelineInput input) throws DITAOTException {
    if (fileInfoFilter == null) {
      fileInfoFilter =
          fileInfo -> fileInfo.format != null && fileInfo.format.equals(ATTR_FORMAT_VALUE_DITAMAP);
    }
  ...
  private void processMap(final FileInfo input) throws DITAOTException {
    ...
    try {
      ...
      transformer.setParameter(
          new QName("file-being-processed"), XdmItem.makeValue(inputFile.getName()));
      ...
      try {
        ...
        throw new DITAOTException(
            "Failed to merge map " + inputFile + ": " + cause.getMessageAndLocation(), e);
    ...
    if (!fileInfo.hasConref) {
      builder.hasConref(
          elements.stream()
              .anyMatch(
                  e ->
                      e.hasAttribute(ATTRIBUTE_NAME_CONREF)
                          || e.hasAttribute(ATTRIBUTE_NAME_CONKEYREF)));
    }
  ...
  private void replace(final FileInfo input) throws DITAOTException {
    ...
    try {
      ...
      throw new DITAOTException(
          "Failed to replace temporary file " + inputFile + ": " + e.getMessage(), e);
    }
  }
}
