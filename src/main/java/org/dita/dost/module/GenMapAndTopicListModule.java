...
/**
 * This class extends AbstractPipelineModule, used to generate map and topic list by parsing all the
 * refered dita files.
 *
...
public final class GenMapAndTopicListModule extends SourceReaderModule {
  ...
  // XXX This is a hack to retain format. A better solution would be to keep the format with the
  // source URI
  ...
  private void parseInputParameters(final AbstractPipelineInput input) {
    ...
    genDebugInfo =
        Boolean.parseBoolean(input.getAttribute(ANT_INVOKER_EXT_PARAM_GENERATE_DEBUG_ATTR));
    ...
    job.setOnlyTopicInMap(
        Boolean.parseBoolean(input.getAttribute(ANT_INVOKER_EXT_PARAM_ONLYTOPICINMAP)));
    ...
    if (ditaInputDir != null) {
      ...
      } else if (ditaInputDir.getPath() != null
          && ditaInputDir.getPath().startsWith(URI_SEPARATOR)) {
    ...
    if (input.getAttribute(ANT_INVOKER_PARAM_RESOURCES) != null) {
      resources =
          Stream.of(input.getAttribute(ANT_INVOKER_PARAM_RESOURCES).split(File.pathSeparator))
    ...
    if (ditaInput.isAbsolute()) {
      ...
      rootFile =
          Objects.requireNonNullElseGet(baseInputDir, () -> basedir.toURI()).resolve(ditaInput);
    }
    ...
    if (input.getAttribute(ANT_INVOKER_PARAM_PROFILING_ENABLED) != null) {
      profilingEnabled =
          Boolean.parseBoolean(input.getAttribute(ANT_INVOKER_PARAM_PROFILING_ENABLED));
    }
    ...
    try {
      ...
      if (currentFile.equals(rootFile)) {
        throw new DITAOTException(
            MessageUtils.getMessage("DOTJ012F", params) + ": " + sax.getMessage(), sax);
      } else if (processingMode == Mode.STRICT) {
        throw new DITAOTException(
            MessageUtils.getMessage("DOTJ013E", params) + ": " + sax.getMessage(), sax);
      } else {
      ...
      if (currentFile.equals(rootFile)) {
        throw new DITAOTException(
            MessageUtils.getMessage("DOTJ012F", params) + ": " + e.getMessage(), e);
      } else if (processingMode == Mode.STRICT) {
        throw new DITAOTException(
            MessageUtils.getMessage("DOTJ013E", params) + ": " + e.getMessage(), e);
      } else {
  ...
  private void processParseResult(final URI currentFile) {
    ...
    for (final Map.Entry<String, KeyDef> e : keydefFilter.getKeysDMap().entrySet()) {
      ...
      if (schemeSet.contains(currentFile)) {
        schemekeydefMap.put(
            key, new KeyDef(key, value.href, value.scope, value.format, currentFile, null));
      }
    }
    ...
    final Set<URI> nonConrefCopytoTargets =
        listFilter.getNonConrefCopytoTargets().stream()
            .map(r -> r.filename)
  ...
  /**
   * Update uplevels if needed. If the parameter contains a {@link
   * org.dita.dost.util.Constants#STICK STICK}, it and anything following it is removed.
   *
  ...
  private void outputResult() throws DITAOTException {
    ...
    // If root input file is marked resource only due to conref or other feature, remove that
    // designation
    ...
    job.setProperty(
        ...
        StringUtils.escapeRegExp(getPrefix(relativeRootFile)));
  ...
  private Map<URI, URI> filterConflictingCopyTo(
      final Map<URI, URI> copyTo, final Collection<FileInfo> fileInfos) {
    final Set<URI> fileinfoTargets =
        fileInfos.stream()
   ...
   * @return either an empty string or a path which ends in {@link java.io.File#separator
   *     File.separator}
   */
  ...
  private void addFlagImagesSetToProperties(final Job prop, final Set<URI> set) {
    ...
    final String fileKey =
        REL_FLAGIMAGE_LIST.substring(0, REL_FLAGIMAGE_LIST.lastIndexOf("list")) + "file";
    prop.setProperty(
        fileKey, REL_FLAGIMAGE_LIST.substring(0, REL_FLAGIMAGE_LIST.lastIndexOf("list")) + ".list");
