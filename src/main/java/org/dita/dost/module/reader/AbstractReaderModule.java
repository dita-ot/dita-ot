...
public abstract class AbstractReaderModule extends AbstractPipelineModuleImpl {
  ...
  @VisibleForTesting final NavigableMap<URI, Reference> waitList = new ConcurrentSkipListMap<>();
  ...
  void parseInputParameters(final AbstractPipelineInput input) {
    ...
    processingMode =
        Optional.ofNullable(input.getAttribute(ANT_INVOKER_EXT_PARAM_PROCESSING_MODE))
    ...
    job.setCrawl(
        Optional.ofNullable(input.getAttribute(ANT_INVOKER_EXT_PARAM_CRAWL))
    ...
    profilingEnabled =
        Optional.ofNullable(input.getAttribute(ANT_INVOKER_PARAM_PROFILING_ENABLED))
  ...
  void readFile(final Reference ref, final URI parseFile) throws DITAOTException {
    ...
    try {
      ...
      if (!exists(currentFile)) {
        ...
        throw new DITAOTException(
            MessageUtils.getMessage("DOTJ078F", params).toString()
                + " Cannot load file: "
                + e.getMessage(),
            e);
      } else if (processingMode == Mode.STRICT) {
        throw new DITAOTException(
            MessageUtils.getMessage("DOTJ079E", params).toString()
                + " Cannot load file: "
                + e.getMessage(),
            e);
      } else {
  ...
  private Collection<FileInfo> getOrCreateFileInfo(
      final Map<URI, Collection<FileInfo>> fileInfos, final URI file) {
    ...
    if (fileInfos.containsKey(f)) {
      ...
      final Collection<FileInfo> prevs =
          job.getFileInfo(fi -> Objects.equals(fi.src, f)).stream()
              .map(
                  prev -> {
  ...
  void init() throws SAXException {
    try {
      final String cls =
          Optional.ofNullable(job.getProperty("temp-file-name-scheme"))
