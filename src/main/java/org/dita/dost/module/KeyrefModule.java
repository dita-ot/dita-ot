...
final class KeyrefModule extends AbstractPipelineModuleImpl {
  ...
  public AbstractPipelineOutput execute(final AbstractPipelineInput input) throws DITAOTException {
    ...
    final Collection<FileInfo> fis =
        job.getFileInfo(fileInfoFilter).stream()
    ...
    if (!fis.isEmpty()) {
      ...
      // Read resources maps
      final Collection<FileInfo> resourceFis =
          job.getFileInfo(
              fi -> fi.isInputResource && Objects.equals(fi.format, ATTR_FORMAT_VALUE_DITAMAP));
  ...
  private List<ResolveTask> collectProcessingTopics(
    ...
    try {
      ...
      final PipelineConfiguration pipe =
          doc.getUnderlyingNode().getConfiguration().makePipelineConfiguration();
      final Receiver receiver =
          new NamespaceReducer(destination.getReceiver(pipe, new SerializationProperties()));
      receiver.open();
  ...
  List<ResolveTask> adjustResourceRenames(final List<ResolveTask> renames) {
    final Map<KeyScope, List<ResolveTask>> scopes =
        renames.stream().collect(Collectors.groupingBy(rt -> rt.scope));
  ...
  KeyScope rewriteScopeTargets(KeyScope scope, Map<URI, URI> rewrites) {
    ...
    for (Map.Entry<String, KeyDef> key : scope.keyDefinition().entrySet()) {
      ...
      final KeyDef newKey =
          new KeyDef(oldKey.keys, href, oldKey.scope, oldKey.format, oldKey.source, oldKey.element);
    ...
    switch (node.getNodeKind()) {
      case ELEMENT -> {
        if (MAP_MAP.matches(node) || MAP_TOPICREF.matches(node)) {
          final List<KeyScope> ss =
              node.attribute(ATTRIBUTE_NAME_KEYSCOPE) != null
            ...
            if (referenceValue != null) {
              for (final KeyScope s : ss) {
                ...
                if (fi != null && fi.hasKeyref) {
                  ...
                  final Optional<ResolveTask> existing =
                      res.stream()
                  ...
                  if (count != 0 && existing.isPresent()) {
                    ...
                    if (resolveTask.out != null) {
                      referenceValue =
                          tempFileNameScheme.generateTempFileName(resolveTask.out.result);
  ...
  private NodeName toNodeName(final QName qName) {
    ...
    return new FingerprintedQName(
        structuredQName.getPrefix(), structuredQName.getURI(), structuredQName.getLocalPart());
  }
  ...
  private boolean isResourceOnly(final XdmNode elem) {
    return elem.select(
            ancestorOrSelf(Predicates.hasAttribute(ATTRIBUTE_NAME_PROCESSING_ROLE))
                ...
                .where(
                    attributeEq(
                        ATTRIBUTE_NAME_PROCESSING_ROLE, ATTR_PROCESSING_ROLE_VALUE_RESOURCE_ONLY)))
        .exists();
  }
   ...
   */
  private ResolveTask processTopic(
      final FileInfo f, final KeyScope scope, final boolean isResourceOnly) {
  ...
  /**
   * Process key references in a topic. Topic is stored with a new name if it's been processed
   * before.
   */
  private void processFile(final ResolveTask r) {
    ...
    final TopicFragmentFilter topicFragmentFilter =
        new TopicFragmentFilter(ATTRIBUTE_NAME_CONREF, ATTRIBUTE_NAME_CONREFEND);
    ...
    try {
      ...
      if (r.out != null) {
        logger.info(
            "Processing "
                + job.tempDirURI.resolve(r.in.uri)
                + " to "
                + job.tempDirURI.resolve(r.out.uri));
        job.getStore()
            .transform(
                job.tempDirURI.resolve(r.in.uri), job.tempDirURI.resolve(r.out.uri), filters);
      } else {
