...
/**
 * DebugAndFilterModule implement the second step in preprocess. It will insert debug information
 * into every dita files and filter out the information that is not necessary.
 *
...
public final class DebugAndFilterModule extends SourceReaderModule {
  ...
  public AbstractPipelineOutput execute(final AbstractPipelineInput input) throws DITAOTException {
    ...
    try {
      ...
      throw new DITAOTException(
          "Exception doing debug and filter module processing: " + e.getMessage(), e);
    }
  ...
  private void outputSubjectScheme() throws DITAOTException {
    try {
      ...
      while (!queue.isEmpty()) {
        ...
        final File tmprel =
            new File(FileUtils.resolve(job.tempDir, parent) + SUBJECT_SCHEME_EXTENSION);
        ...
        generateScheme(
            new File(job.tempDir, parent.getPath() + SUBJECT_SCHEME_EXTENSION), parentRoot);
      }
    } catch (final RuntimeException e) {
      throw e;
    } catch (final Exception e) {
      ...
      if (!StringUtils.isEmptyString(value = pe.getAttribute(ATTRIBUTE_NAME_KEYREF))) {
        ...
        for (int i = 0; i < pList.getLength(); i++) {
          ...
          if (tmpnode.getNodeType() == Node.ELEMENT_NODE
              && searchForKey(target, ((Element) tmpnode).getAttribute(ATTRIBUTE_NAME_KEYS))
                  != null) {
            continue;
          }
        ...
        if (target != null) {
          ...
          for (int i = 0; i < pList.getLength(); i++) {
            ...
            if (tmpnode.getNodeType() == Node.ELEMENT_NODE
                && searchForKey(pe, ((Element) tmpnode).getAttribute(ATTRIBUTE_NAME_KEYS))
                    != null) {
              continue;
            }
  ...
  private static String getRelativePathFromOut(final File overflowingFile, final Job job) {
    ...
    final File finalRelativePathName =
        FileUtils.getRelativePath(finalOutFilePathName, outputPathName);
  ...
  private static boolean isOutFile(final File filePathName, final File inputMap) {
    final File relativePath =
        FileUtils.getRelativePath(inputMap.getAbsoluteFile(), filePathName.getAbsoluteFile());
