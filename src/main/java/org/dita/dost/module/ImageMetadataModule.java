...
final class ImageMetadataModule extends AbstractPipelineModuleImpl {
  ...
  public AbstractPipelineOutput execute(final AbstractPipelineInput input) throws DITAOTException {
    ...
    final Collection<FileInfo> images =
        job.getFileInfo(
            f ->
                ATTR_FORMAT_VALUE_IMAGE.equals(f.format)
                    || ATTR_FORMAT_VALUE_HTML.equals(f.format));
    if (!images.isEmpty()) {
      final File outputDir = new File(input.getAttribute(ANT_INVOKER_EXT_PARAM_OUTPUTDIR));
      final Predicate<FileInfo> filter =
          fileInfoFilter != null
              ? fileInfoFilter
      ...
      if (parallel) {
        final Pool<ImageMetadataFilter> pool =
            new Pool<>(
                () -> {
