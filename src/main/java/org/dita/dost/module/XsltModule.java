 ...
 *
 * <p>The module matches Ant's XSLT task with the following exceptions:
 *
 * <ul>
 *   <li>If source and destination directories are same, transformation results are saved to a
 *       temporary file and the original source file is replaced after a successful transformation.
 *   <li>If no {@code extension} attribute is set, the target file extension is the same as the
...
public final class XsltModule extends AbstractPipelineModuleImpl {
  ...
  public AbstractPipelineOutput execute(AbstractPipelineInput input) throws DITAOTException {
    ...
    if (in != null) {
      ...
      try {
        final List<Entry<File, File>> tmps =
            includes.stream()
                .parallel()
                .map(
                    include -> {
  ...
  private void transform(final File in, final File out, final XsltTransformer t)
    ...
    final File tmp =
        same
            ? new File(out.getAbsolutePath() + ".tmp" + Long.toString(System.currentTimeMillis()))
            : out;
    if (same) {
      logger.info("Processing " + in.toURI());
