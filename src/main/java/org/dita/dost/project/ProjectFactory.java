...
public class ProjectFactory {
  ...
  private static final ObjectReader yamlReader =
      new YAMLMapper()
          .reader()
          .forType(ProjectBuilder.class)
          .with(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY);
                                    ...
                                    final Context pub =
                                        src.contexts().stream()
                                            .filter(
                                                cntx ->
                                                    Objects.equals(
                                                        cntx.id(), deliverable.context().idref()))
                                            .findAny()
                                            .orElseThrow(
                                                () ->
                              ...
                              .map(
                                  idref -> {
                                    final Publication pub =
                                        src.publications().stream()
                                            .filter(
                                                publ ->
                                                    Objects.equals(
                                                        publ.id(),
                                                        deliverable.publication().idref()))
                                            .findAny()
                                            .orElseThrow(
                                                () ->
                                                    new RuntimeException(
  ...
  private Project load(final URI file, final Set<URI> processed)
    ...
    return resolveIncludes(
        project, file, ImmutableSet.<URI>builder().addAll(processed).add(file).build());
  }
  ...
  private Project resolveIncludes(final Project project, final URI base, final Set<URI> processed)
    ...
    if (project.includes() != null) {
      for (final ProjectRef projectRef : project.includes()) {
        final URI href =
            projectRef.href().isAbsolute() ? projectRef.href() : base.resolve(projectRef.href());
