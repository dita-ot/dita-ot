...
public class ConversionArguments extends Arguments {
  ...
  private Map<String, Object> handleArgDefine(final String arg, final Deque<String> args) {
    ...
    if (RESERVED_PROPERTIES.containsKey(entry.getKey())) {
      throw new BuildException(
          "Property "
              + entry.getKey()
              + " cannot be set with -D, use "
              + RESERVED_PROPERTIES.get(entry.getKey())
              + " instead");
    }
  ...
  private void handleArgResource(
      final String arg, final Deque<String> args, final Argument argument) {
    final Map.Entry<String, String> entry = parse(arg, args);
    if (entry.getValue() == null) {
  ...
  void printUsage(final boolean compact) {
    ...
    if (!compact) {
      ...
      final List<Element> params =
          toList(Plugins.getPluginConfiguration().getElementsByTagName("param"));
      params.stream()
          ...
          .forEach(
              arg -> {
                if (arg instanceof FileArgument) {
                  ...
                  buf.options(
                      null, arg.property, String.join("|", ((EnumArgument) arg).values), arg.desc);
                } else {
