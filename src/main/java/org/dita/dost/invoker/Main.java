...
/**
 * Command line entry point into DITA-OT. This class is entered via the canonical `public static
 * void main` entry point and reads the command line arguments. It then assembles and executes an
 * Ant project.
 */
public class Main extends org.apache.tools.ant.Main implements AntMain {
  ...
  private static final Map<String, String> RESERVED_PARAMS =
      ImmutableMap.of(
          "output.dir", "output",
          "transtype", "transtype",
          "args.input", "input",
  ...
  /**
   * Creates a new instance of this class using the arguments specified, gives it any extra user
   * properties which have been specified, and then runs the build using the classloader provided.
   *
   * @param args Command line arguments. Must not be <code>null</code>.
   * @param additionalUserProperties Any extra properties to use in this build. May be <code>null
   *     </code>, which is the equivalent to passing in an empty set of properties.
   * @param coreLoader Classloader used for core classes. May be <code>null</code> in which case the
   ...
   * @param additionalUserProperties properties to set beyond those that may be specified on the
   *     args list
  ...
  /**
   * This operation is expected to call {@link System#exit(int)}, which is what the base version
   * does. However, it is possible to do something else.
   *
  ...
  /**
   * Command line entry point. This method kicks off the building of a project object and executes a
   * build using either a given target or the default target.
   *
   * @param args Command line arguments. Must not be <code>null</code>.
   */
  ...
  /**
   * Process command line arguments. When ant is started from Launcher, launcher-only arguments do
   * not get passed through to this routine.
   *
  ...
  private void processArgs(final String[] arguments) {
    ...
    if (args instanceof PluginsArguments) {
      ...
      if (installArgs.installFile != null) {
        targets.add("install");
        final File f =
            new File(installArgs.installFile.replace('/', File.separatorChar)).getAbsoluteFile();
        if (f.exists()) {
  ...
  List<Map<String, Object>> collectProperties(
    ...
    final List<Map<String, Object>> projectProps =
        zipWithIndex(project.deliverables())
            .filter(
                entry ->
            ...
            .map(
                entry -> {
                            ...
                            if (param.value() != null) {
                              final Argument argument =
                                  ArgumentParser.getPluginArguments()
                                      .getOrDefault(
                                          param.name(), new StringArgument(param.name(), null));
                              ...
                              if (argument != null
                                  && (argument instanceof FileArgument
                                      || argument instanceof AbsoluteFileArgument)) {
                                if (param.href() != null) {
                  ...
                  final List<org.dita.dost.project.Project.Deliverable.Profile.DitaVal> ditavals =
                      Stream.concat(
    ...
    if (runDeliverable != null && projectProps.isEmpty()) {
      printErrorMessage(
          String.format(locale.getString("project.error.deliverable_not_found"), runDeliverable));
      throw new BuildException("");
    }
  ...
  protected Path getOutputDir(
    ...
    URI outputDir =
        new File(props.getOrDefault(ANT_OUTPUT_DIR, "out").toString()).getAbsoluteFile().toURI();
    outputDir =
        outputDir.getPath().endsWith("/")
            ? outputDir
            : URLUtils.setPath(outputDir, outputDir.getPath() + "/");
    return Paths.get(
        deliverable.output() != null ? outputDir.resolve(deliverable.output()) : outputDir);
  }

  private org.dita.dost.project.Project readProjectFile(final File projectFile)
      throws BuildException {
    if (!projectFile.exists()) {
      printErrorMessage(
          String.format(locale.getString("project.error.project_file_not_found"), projectFile));
      throw new BuildException("");
    }
    try {
  ...
  private void printDeliverables(final File projectFile) {
    final List<Map.Entry<String, String>> pairs =
        readProjectFile(projectFile).deliverables().stream()
            .filter(deliverable -> deliverable.id() != null)
            .map(deliverable -> pair(deliverable.id(), deliverable.name()))
            .collect(Collectors.toList());
    final int length =
        pairs.stream().map(Map.Entry::getKey).map(String::length).reduce(Integer::max).orElse(0);
   ...
   *
   * <p>Takes the given target as a suffix to append to each parent directory in search of a build
   * file. Once the root of the file-system has been reached <code>null</code> is returned.
   *
  ...
  /**
   * Executes the build. If the constructor for this instance failed (e.g. returned after issuing a
   * warning), this method returns immediately.
   *
   * @param coreLoader The classloader to use to find core classes. May be <code>null</code>, in
   ...
   */
  private void runBuild(final ClassLoader coreLoader, Map<String, Object> definedProps)
      throws BuildException {
    ...
    try {
      ...
      try {
        ...
        if (args.threadPriority != null) {
          try {
            project.log(
                "Setting Ant's thread priority to " + args.threadPriority, Project.MSG_VERBOSE);
  ...
  /**
   * Adds the listeners specified in the command line arguments, along with the default listener, to
  ...
  private void printVersion() throws BuildException {
    System.out.println(
        String.format(locale.getString("version"), Configuration.configuration.get("otversion")));
  }
}
