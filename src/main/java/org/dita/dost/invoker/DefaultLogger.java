...
/**
 * Writes build events to a PrintStream. Currently, it only writes which targets are being executed,
 * and any messages that get logged.
 */
class DefaultLogger implements BuildLogger {
   ...
   *
   * <p>Only messages with a message level lower than or equal to the given level should be written
   * to the log.
   *
   * <p>Constants for the message levels are in the {@link Project Project} class. The order of the
   * levels, from least to most verbose, is <code>MSG_ERR</code>, <code>MSG_WARN</code>, <code>
   * MSG_INFO</code>, <code>MSG_VERBOSE</code>, <code>MSG_DEBUG</code>.
   *
   * <p>The default message level for DefaultLogger is Project.MSG_ERR.
   *
   ...
   *
   * @param emacsMode <code>true</code> if output is to be unadorned so that emacs and other editors
  ...
  private static void throwableMessage(
      final StringBuilder m, final Throwable error, final boolean verbose) {
  ...
  /**
   * Prints whether the build succeeded or failed, any errors the occurred during the build, and how
   * long the build took.
   *
  ...
  /**
   * This is an override point: the message that indicates whether a build failed. Subclasses can
   * change/enhance the message.
   *
  ...
  /**
   * This is an override point: the message that indicates that a build succeeded. Subclasses can
   * change/enhance the message.
   *
  ...
  /**
   * Logs a message to say that the target has started if this logger allows information-level
   * messages.
   *
   * @param event An event with any relevant extra information. Must not be <code>null</code>.
   */
  ...
  /**
   * Logs a message, if the priority is suitable. In non-emacs mode, task level messages are
   * prefixed by the task name which is right-justified.
   *
   ...
   * @param stream A PrintStream to print the message to. Must not be <code>null</code>.
