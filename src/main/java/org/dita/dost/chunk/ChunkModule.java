...
public class ChunkModule extends AbstractPipelineModuleImpl {
  ...
  private void rewriteLinks(
      final Map<URI, URI> combineRewriteMap, final Map<URI, URI> splitRewriteMap) {
    if (!combineRewriteMap.isEmpty() && !splitRewriteMap.isEmpty()) {
      ...
      (parallel ? topics.parallelStream() : topics.stream())
          .forEach(
              fi -> {
                try {
  ...
  private void rewriteTopicLinks(
      final Document doc, final URI src, final Map<URI, URI> rewriteMap) {
  ...
  private Map<URI, URI> processCombine(
      final URI mapFile, final Document mapDoc, final List<ChunkOperation> chunks)
      throws IOException {
    if (chunks.stream().anyMatch(c -> c.operation().equals(COMBINE))) {
      ...
      final List<ChunkOperation> rewrittenChunks =
          rewriteCombineChunks(mapFile, mapDoc, normalTopicRefs, rewriteMap, chunks);
  ...
  private Map<URI, URI> processSplit(
      final URI mapFile, final Document mapDoc, final List<ChunkOperation> chunks)
      throws IOException {
    if (chunks.stream().anyMatch(c -> c.operation().equals(SPLIT))) {
      ...
      for (ChunkOperation chunk : rewrittenChunks) {
        if (chunk.operation() == SPLIT) {
          ...
          var topicrefs =
              splitNestedTopic(fileInfo, doc.getDocumentElement(), chunk.topicref(), rewriteMap);
  ...
  private List<ChunkOperation> rewriteSplitChunks(
      final URI mapFile, final Document mapDoc, List<ChunkOperation> chunks) {
    final Set<URI> normalTopicRefs = getNormalTopicRefs(mapFile, mapDoc);
  ...
  private void processSplitTopic(
      ChunkOperation chunk, Document doc, List<Element> topicrefs, FileInfo fileInfo)
      throws IOException {
  ...
  private void processSplitDitabase(ChunkOperation chunk, List<Element> topicrefs)
      throws IOException {
  ...
  private List<Element> splitNestedTopic(
    ...
    return getChildElements(topic, TOPIC_TOPIC).stream()
        .map(
            nestedTopic -> {
              final Element nestedTopicref =
                  topicref.getOwnerDocument().createElement(MAP_TOPICREF.localName);
              nestedTopicref.setAttribute(ATTRIBUTE_NAME_CLASS, MAP_TOPICREF.toString());

              final List<Element> childTopicrefs =
                  splitNestedTopic(fileInfo, nestedTopic, nestedTopicref, rewriteMap);
              ...
              final FileInfo adoptedFileInfo =
                  new Builder(fileInfo).uri(tmp).result(result).build();
              job.add(adoptedFileInfo);
              try {
              ...
              nestedTopicref.setAttribute(
                  ATTRIBUTE_NAME_HREF, tmp.resolve(".").relativize(tmp).toString());
  ...
  private void removeChunkSources(
      final Set<URI> normalTopicRefs, final List<ChunkOperation> chunks) {
    ...
    final Set<URI> added =
        destinations.stream().filter(dst -> !sources.contains(dst)).collect(Collectors.toSet());
    added.forEach(
        tmp -> {
          if (job.getFileInfo(tmp) == null) {
            final FileInfo src =
                chunks.stream()
                    ...
                    .findAny()
                    .flatMap(
                        chunk -> Optional.ofNullable(job.getFileInfo(removeFragment(chunk.src()))))
                    .orElse(null);
            final Builder builder = src != null ? FileInfo.builder(src) : FileInfo.builder();
            final URI dstRel = job.tempDirURI.relativize(tmp);
            //                final URI result = src != null && src.result != null ?
            // src.result.resolve(tmp) : toDirURI(job.getOutputDir()).resolve(tmp);
            final FileInfo dstFi =
                builder
                    .uri(dstRel)
  ...
  private Set<URI> collectResources(
      List<ChunkOperation> chunks, final Function<ChunkOperation, URI> pick) {
  ...
  private void collectResources(
      List<ChunkOperation> chunks, final Function<ChunkOperation, URI> pick, Set<URI> res) {
    for (ChunkOperation chunk : chunks) {
  ...
  private void generateChunks(List<ChunkOperation> chunks, Map<URI, URI> rewriteMap) {
    ...
    //                final FileInfo src = chunk.src != null ?
    // job.getFileInfo(removeFragment(chunk.src, null)) : null;
    //                final FileInfo.Builder builder = src != null ? FileInfo.builder(src) :
    // FileInfo.builder();
  ...
  private List<ChunkOperation> rewriteCombineChunks(
    ...
    return chunks.stream()
        ...
        .map(
            chunk ->
                rewriteCombineChunk(mapFile, mapDoc, normalTopicRefs, rewriteMap, chunk).build())
  ...
  private ChunkBuilder rewriteCombineChunk(
    ...
    final ChunkBuilder builder =
        new ChunkBuilder(rootChunk.operation())
            .topicref(rootChunk.topicref())
            .src(rootChunk.src())
            .dst(dst)
            .id(id);
    for (ChunkOperation child : rootChunk.children()) {
      final ChunkBuilder childBuilder =
          rewriteChunkChild(rewriteMap, Objects.requireNonNull(dst), child);
  ...
  private ChunkBuilder rewriteChunkChild(
      final Map<URI, URI> rewriteMap, final URI rootChunkDst, final ChunkOperation chunk) {
    String id;
    ...
    final ChunkBuilder builder =
        new ChunkBuilder(chunk.operation())
  ...
  private String getRootTopicId(final URI src) {
    ...
    try {
      ...
      final Step<XdmNode> firstTopicId =
          descendant(TOPIC_TOPIC.matcher()).first().then(attribute(ATTRIBUTE_NAME_ID));
  ...
  private void mergeTopic(
      final ChunkOperation rootChunk, final ChunkOperation chunk, final Element dstTopic)
      throws IOException {
    for (ChunkOperation child : chunk.children()) {
      Element added;
      if (child.src() != null) {
    ...
    if (chunk.equals(COMBINE.name)) {
      ...
      if (MAP_MAP.matches(elem)) {
        ...
        if (hrefNode != null) {
          ...
          logger.warn(
              MessageUtils.getMessage("DOTJ086W", elem.getTagName()).setLocation(elem).toString());
        }
        for (Element child : getChildElements(elem, MAP_TOPICREF)) {
  ...
  private List<ChunkBuilder> collectCombineChunks(final URI mapFile, final Element elem) {
    if (!elem.getAttribute(ATTRIBUTE_NAME_CHUNK).isEmpty()) {
      logger.warn(
          MessageUtils.getMessage("DOTJ087W", elem.getAttribute(ATTRIBUTE_NAME_CHUNK))
              .setLocation(elem)
              .toString());
