...
<xsl:call-template name="gen-id"/><xsl:apply-templates/><xsl:if test="not(ancestor::li[contains(@class, ' topic/li ')])">\par\pard\li360\fi-180</xsl:if>
</xsl:template>
...
<xsl:variable name="depth"><xsl:value-of select="count(ancestor::li[contains(@class, ' topic/li ')])"/></xsl:variable>
