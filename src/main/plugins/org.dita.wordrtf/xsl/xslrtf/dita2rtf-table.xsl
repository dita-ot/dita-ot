...
\par }</xsl:text>
  ...
  <xsl:template match="*[contains(@class,' topic/table ')]/*[contains(@class,' topic/title ')]">
    ...
    <xsl:call-template name="gen-id"/>\pard \plain\s9 \qc\f4\fs24\b <xsl:choose>
      <!-- Hungarian: "1.
Table " -->
...
name="get-ascii">
    ...
    </xsl:call-template>\par \plain\s0 \qj\f2\fs24
  </xsl:template>
...
\trftsWidth1\trautofit1\trpaddl108\trpaddr108\trpaddfl3\trpaddfr3</xsl:text>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>\plain \s0\f4\fs24 <xsl:value-of select="$table-row-default-align"/>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:with-param name="num" select="$empty-cell-num"/>
  ...
  <xsl:template match="*[contains(@class,' topic/colspec ')]" mode="count-rowwidth">
    ...
    <xsl:variable name="thiswidth">
      ...
      <xsl:choose>
        <xsl:when test="@colwidth and contains(@colwidth,'*') and not(@colwidth='*')">
  ...
  <xsl:template match="*[contains(@class,' topic/entry ')]" mode="emit-cell-style">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:value-of select="table-cell-default-valign"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:otherwise>
      </xsl:choose>
    ...
    <xsl:variable name="totalwidth">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="$col-max-num"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="thiswidth">
...
we need to emit merged entry style -->
    ...
    <xsl:if test="@morerows and $copy-style='false'">
      <xsl:text>\clvmgf</xsl:text>
    </xsl:if>
    ...
    <xsl:value-of select="$thiswidth-twips"/>
...
\li0\fi0\ri0\nowidctlpar\intbl\aspalpha\aspnum\faauto\adjustright\rin0\lin0
  ...
  <xsl:template match="*[contains(@class,' topic/stentry ')]" mode="count-rowwidth">
    <xsl:param name="totalcols">0</xsl:param>
    <!-- Total counted columns so far -->
    <xsl:choose>
