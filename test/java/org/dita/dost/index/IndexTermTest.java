...
public class IndexTermTest {
	...
	public void testGetSubTerms() {
		...
		assertEquals(2, ((IndexTerm) nested.getSubTerms().get(0)).getSubTerms().size());
	}
	...
	public void testSortSubTerms() {
		...
		for (final IndexTerm i: ((List<IndexTerm>) root.getSubTerms())) {
	...
	public void testUpdateSubTerm() {
		...
		for (final IndexTerm s: ((List<IndexTerm>) single.getSubTerms())) {
		...
		more.updateSubTerm();
		for (final IndexTerm m: ((List<IndexTerm>) more.getSubTerms())) {
