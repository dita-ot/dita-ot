...
\par\pard\plain \qj \fi-420\li<xsl:value-of select="$li-num"/><xsl:if test="ancestor::*[contains(@class,' topic/table ') or contains(@class,' topic/simpletable ')]">\intbl</xsl:if>\jclisttab\tx<xsl:value-of select="$li-num"/>\ls<xsl:value-of select="$listnum"/><xsl:if test="$depth &gt; 0">\ilvl<xsl:value-of select="$depth"/></xsl:if><xsl:text> </xsl:text><xsl:if test="@importance='optional'"><xsl:call-template name="getString"><xsl:with-param name="stringName" select="'Optional'"/></xsl:call-template><xsl:call-template name="getString"><xsl:with-param name="stringName" select="'ColonSymbol'"/></xsl:call-template><xsl:text> </xsl:text></xsl:if><xsl:if test="@importance='required'"><xsl:call-template name="getString"><xsl:with-param name="stringName" select="'Required'"/></xsl:call-template><xsl:call-template name="getString"><xsl:with-param name="stringName" select="'ColonSymbol'"/></xsl:call-template><xsl:text> </xsl:text></xsl:if><xsl:apply-templates/></xsl:template>
