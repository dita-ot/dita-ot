...
<xsl:param name="PATHTOMAP" select="''"/>
...
<xsl:template name="countDirectoriesInPath">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise><xsl:value-of select="$currentCount"/></xsl:otherwise>
  </xsl:choose>
</xsl:template>
    ...
    <xsl:when test="contains($originalLink,'\')">
      <xsl:call-template name="simplifyLink">
        <xsl:with-param name="originalLink">
          <xsl:value-of select="substring-before($originalLink,'\')"/>/<xsl:value-of select="substring-after($originalLink,'\')"/>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:when>
    <xsl:when test="not(contains($originalLink,'../'))">
      <xsl:value-of select="$buildLink"/><xsl:value-of select="$originalLink"/>
    </xsl:when>
...
<xsl:template name="MoveBackToBase">
  ...
  <xsl:param name="dirsLeft"><xsl:value-of select="$DIRS-IN-MAP-PATH"/></xsl:param>
  <xsl:param name="remainingPath"><xsl:value-of select="$PATHTOMAP"/></xsl:param>
  
  <xsl:choose>
    <xsl:when test="$saveDirs>=$dirsLeft"><xsl:value-of select="$remainingPath"/></xsl:when>
...
<xsl:template name="getPathBackToBase">
  <xsl:param name="path"/>                <!-- The href value -->
  <xsl:variable name="directoriesBack">   <!-- Number of directories above the map that $path travels -->
    <xsl:call-template name="countRelpaths">
      <xsl:with-param name="path" select="$path"/>
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="moveToBase">        <!-- Path from the closest common ancestor, back to the base -->
    <xsl:call-template name="MoveBackToBase">
      <xsl:with-param name="saveDirs"><xsl:value-of select="$directoriesBack"/></xsl:with-param>
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="pathWithoutRelPaths"> <!-- Path from the common ancestor, to the target file -->
    <xsl:call-template name="removeRelPaths">
      <xsl:with-param name="path" select="$path"/>
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="backToCommon">      <!-- Path from the target file, to the common ancestor -->
    <xsl:call-template name="pathBackToMapDirectory">
...
<xsl:template name="pathBackToMapDirectory">
  <xsl:param name="path"/>  <!-- Portion of the href that still needs to be evaluated -->
  <xsl:param name="back"/>  <!-- Relpath builds up as we go; add ../ here each time a directory is removed -->
