...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <xsl:when test="//*[contains(@class,' bkinfo/bkinfo ')][1]">
                        <fo:block xsl:use-attribute-sets="__frontmatter">
                            ...
                            <fo:block xsl:use-attribute-sets="__frontmatter__title">
                                <xsl:apply-templates select="//*[contains(@class,' bkinfo/bkinfo ')]/*[contains(@class,' topic/title ')]/node()"/>
                            </fo:block>

                            <!-- set the subtitle -->
                            <xsl:apply-templates select="//*[contains(@class,' bkinfo/bkinfo ')]/*[contains(@class,' bkinfo/bktitlealts ')]/*[contains(@class,' bkinfo/bksubtitle ')]"/>
    ...
    <xsl:template match="*[contains(@class, ' bkinfo/bkowner ')]">
        <fo:block-container xsl:use-attribute-sets="__frontmatter__owner__container">
            <fo:block >
                <fo:inline>
                    <xsl:apply-templates select="organization/orgname"/>
                </fo:inline>
                    &#xA0;
                <fo:inline>
                    <xsl:apply-templates select="organization/address"/>
                </fo:inline>
            </fo:block>
        </fo:block-container>
    </xsl:template>
